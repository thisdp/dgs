...
function dgsSetLayer(dgsEle, layer, forceDetatch)
	if not(dgsIsType(dgsEle)) then
		error(dgsGenAsrt(dgsEle, "dgsSetLayer", 1, "dgs-dxelement"))
	end
	...
	if not(layerBuiltIn[layer]) then
		error(dgsGenAsrt(layer, "dgsSetLayer", 2, "string", "top/center/bottom"))
	end

	local parent = dgsElementData[dgsEle].parent
	local hasParent = isElement(parent)

	if hasParent and not forceDetatch then
		return false
	end
...
function dgsGetLayer(dgsEle)
	if not(dgsIsType(dgsEle)) then
		error(dgsGenAsrt(dgsEle, "dgsGetLayer", 1, "dgs-dxelement"))
	end
...
function dgsSetCurrentLayerIndex(dgsEle, index)
	if not(dgsIsType(dgsEle)) then
		error(dgsGenAsrt(dgsEle, "dgsSetCurrentLayerIndex", 1, "dgs-dxelement"))
	end

	if not(type(index) == "number") then
		error(dgsGenAsrt(index, "dgsSetCurrentLayerIndex", 2, "number"))
	end
...
function dgsGetCurrentLayerIndex(dgsEle)
	if not(dgsIsType(dgsEle)) then
		error(dgsGenAsrt(dgsEle, "dgsGetCurrentLayerIndex", 1, "dgs-dxelement"))
	end
...
function dgsGetLayerElements(layer)
	if not(layerBuiltIn[layer]) then
		error(dgsGenAsrt(layer, "dgsGetLayerElements", 1, "string", "top/center/bottom"))
	end
...
function dgsGetChild(dgsEle, id)
	if not(dgsIsType(dgsEle)) then
		error(dgsGenAsrt(dgsEle, "dgsGetChild", 1, "dgs-dxelement"))
	end

	if not(type(id) == "number") then
		error(dgsGenAsrt(id, "dgsGetChild", 2, "number"))
	end
...
function dgsGetChildren(dgsEle)
	if not(dgsIsType(dgsEle)) then
		error(dgsGenAsrt(dgsEle, "dgsGetChildren", 1, "dgs-dxelement"))
	end
...
function dgsGetParent(dgsEle)
	if not(dgsIsType(dgsEle)) then
		error(dgsGenAsrt(dgsEle, "dgsGetParent", 1, "dgs-dxelement"))
	end
...
function dgsSetParent(child, newParent, nocheckRoot, noUpdatePosSize)
	if newParent == resourceRoot then
		newParent = nil
	end

	if not(dgsIsType(child)) then
		error(dgsGenAsrt(child, "dgsSetParent", 1, "dgs-dxelement"))
	end

	if not(not dgsElementData[child] or not dgsElementData[child].attachTo) then
		error(dgsGenAsrt(child, "dgsSetParent", 1, _, _, _, "attached dgs element can not have a parent"))
	end

	if not dgsElementData[child] then
		dgsElementData[child] = {}
	end
	...
	if isElement(newParent) then
		if not dgsIsType(newParent) then
			return
		end
		...
		if not dgsElementData[newParent].children then
			dgsElementData[newParent].children = {}
		end
...
function dgsBringToFront(dgsEle, mouse, dontMoveParent, dontFocus)
	...
	if not(eleType) then
		error(dgsGenAsrt(dgsEle, "dgsBringToFront", 1, "dgs-dxelement"))
	end
...
function dgsMoveToBack(dgsEle)
	if not(dgsIsType(dgsEle)) then
		error(dgsGenAsrt(dgsEle, "dgsMoveToBack", 1, "dgs-dxelement"))
	end
...
function dgsGetType(dgsEle, dgsTypeOnly)
	if dgsTypeOnly then
		if isElement(dgsEle) then
			return dgsElementType[dgsEle], dgsType[dgsElementType[dgsEle] or ""]
		end
	else
		if isElement(dgsEle) then
			return tostring(dgsElementType[dgsEle] or getElementType(dgsEle))
		end
...
function dgsIsType(dgsEle, isType)
	if isType then
		if isElement(dgsEle) then
			...
			if isType == (eleData and eleData.asPlugin) then
				return true
			end

			if isType == dgsElementType[dgsEle] then
				return true
			end

			if _G[isType] and _G[isType][dgsElementType[dgsEle]] then
				return true
			end
...
function dgsGetPluginType(dgsEle)
	return dgsEle and (dgsElementData[dgsEle] and dgsElementData[dgsEle].asPlugin or false) or dgsGetType(dgsEle)
end
...
function dgsRegisterProperty(eleType, propertyName, propertyArgTemplate)
	if not(type(eleType) == "string") then
		error(dgsGenAsrt(eleType, "dgsRegisterProperty", 1, "string"))
	end

	if not(type(propertyName) == "string") then
		error(dgsGenAsrt(propertyName, "dgsRegisterProperty", 2, "string"))
	end

	if not(type(propertyArgTemplate) == "table") then
		error(dgsGenAsrt(propertyArgTemplate, "dgsRegisterProperty", 3, "table"))
	end
	...
	if _G[eleType] then
		...
		if not dgsElementPropertyList[eleType] then
			dgsElementPropertyList[eleType] = {}
		end
...
function dgsRegisterProperties(eleType, propertyList)
	if not(type(eleType) == "string") then
		error(dgsGenAsrt(eleType, "dgsRegisterProperties", 1, "string"))
	end

	if not(type(propertyList) == "table") then
		error(dgsGenAsrt(propertyList, "dgsRegisterProperties", 2, "table"))
	end
...
function dgsListPropertyTypes(propertyTemplateValue)
	if type(propertyTemplateValue) ~= "number" then
		return type(propertyTemplateValue)
	end
...
function dgsCheckProperty(dgsElement, propertyName, propertyValue)
	...
	if propertyCheck[propertyName] then
		...
		if not propertyCheck.__Special then
			return true
		end
...
function dgsGetRegisteredProperties(eleType, withArgTemplate)
	if not(type(eleType) == "string") then
		error(dgsGenAsrt(eleType, "dgsGetRegisteredProperties", 1, "string"))
	end
...
function dgsSetData(dgsEle, key, value, nocheck)
	...
	if not (isElement(dgsEle) and dgsType and key) then
		return false
	end
	...
	if translationListener and translationListener[key] then
		if dgsOnTranslationUpdate[dgsEle] then
			dgsOnTranslationUpdate[dgsType](dgsEle, key, value, translationListener[key])
		else
			dgsOnTranslationUpdate.default(dgsEle, key, value, translationListener[key])
		end
	end

	if dataHandler then
		dataHandler(dgsEle, key, value, oldValue)
	end

	if eleData.propertyListener and eleData.propertyListener[key] then
		dgsTriggerEvent("onDgsPropertyChange", dgsEle, key, value, oldValue)
	end
...
function dgsAddPropertyListener(dgsEle, propertyNames)
	...
	if not(dgsIsType(dgsEle) or isTable) then
		error(dgsGenAsrt(dgsEle, "dgsAddPropertyListener", 1, "dgs-dxelement/table"))
	end
...
function dgsRemovePropertyListener(dgsEle, propertyNames)
	...
	if not(dgsIsType(dgsEle) or isTable) then
		error(dgsGenAsrt(dgsEle, "dgsRemovePropertyListener", 1, "dgs-dxelement/table"))
	end
...
function dgsGetListenedProperties(dgsEle)
	if not(dgsIsType(dgsEle)) then
		error(dgsGenAsrt(dgsEle, "dgsGetListenedProperties", 1, "dgs-dxelement"))
	end
...
function checkCompatibility(dgsEle, key, sResRoot)
	...
	if getElementData(resourceRoot, "DGS-enableCompatibilityCheck") then
		return (compatibility[eleTyp] and compatibility[eleTyp][key]) or compatibility[key] or key
	end
...
function dgsSetProperty(dgsEle, key, value, ...)
	...
	if not(dgsIsType(dgsEle) or isTable) then
		error(dgsGenAsrt(dgsEle, "dgsSetProperty", 1, "dgs-dxelement/table"))
	end
	...
	if isTable then
		...
		if key == "functions" then
			if value then
				...
				if not fnc then
					error("Bad argument @dgsSetProperty at argument 2, failed to load function ("..err..")")
				end
...
function dgsGetProperty(dgsEle, key)
	if not(dgsIsType(dgsEle)) then
		error(dgsGenAsrt(dgsEle, "dgsGetProperty", 1, "dgs-dxelement"))
	end
...
function dgsSetProperties(dgsEle, theTable)
	...
	if not(dgsIsType(dgsEle) or isTable) then
		error(dgsGenAsrt(dgsEle, "dgsSetProperties", 1, "dgs-dxelement/table"))
	end

	if not(type(theTable) == "table") then
		error(dgsGenAsrt(theTable, "dgsSetProperties", 2, "table"))
	end
...
function dgsGetProperties(dgsEle, properties)
	if not(dgsIsType(dgsEle) or isTable) then
		error(dgsGenAsrt(dgsEle, "dgsGetProperties", 1, "dgs-dxelement/table"))
	end

	if not(not properties or type(properties) == "table") then
		error(dgsGenAsrt(properties, "dgsGetProperties", 2, "table/none"))
	end
...
function dgsSetPropertyInherit(dgsEle, key, value, ...)
	...
	if not(dgsIsType(dgsEle) or isTable) then
		error(dgsGenAsrt(dgsEle, "dgsSetPropertyInherit", 1, "dgs-dxelement/table"))
	end
...
function dgsAddEasingFunction(name, str, isOverWrite)
	if not(type(name) == "string") then
		error(dgsGenAsrt(name, "dgsAddEasingFunction", 1, "string"))
	end

	if not(type(str) == "string") then
		error(dgsGenAsrt(str, "dgsAddEasingFunction", 2, "string"))
	end

	if easingBuiltIn[name] then
		error(dgsGenAsrt(name, "dgsAddEasingFunction", 1, _, _, "duplicated name with built-in easing function ("..name..")"))
	end

	if not isOverWrite and dgsEasingFunction[name] then
		error(dgsGenAsrt(name, "dgsAddEasingFunction", 1, _, _, "this name has been used ("..name..")"))
	end
	...
	if not fnc then
		error(dgsGenAsrt(fnc, "dgsAddEasingFunction", 2, _, _, _, "Failed to load function:"..err))
	end
...
function dgsRemoveEasingFunction(name)
	if not(type(name) == "string") then
		error(dgsGenAsrt(name, "dgsRemoveEasingFunction", 1, "string"))
	end
...
function dgsEasingFunctionExists(name)
	if not(type(name) == "string") then
		error(dgsGenAsrt(name, "dgsEasingFunctionExists", 1, "string"))
	end
