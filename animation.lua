...
function dgsAnimTo(...)
	...
	if not(type(dgsEle) == "table" or dgsIsType(dgsEle)) then
		error(dgsGenAsrt(dgsEle, "dgsAnimTo", 1, "dgs-dxelement/table"))
	end

	if not(type(property) == "string") then
		error(dgsGenAsrt(property, "dgsAnimTo", 2, "string"))
	end

	easing = easing or "Linear"

	if not(dgsEasingFunctionExists(easing)) then
		error(dgsGenAsrt(easing, "dgsAnimTo", 4, _, "easing function doesn't exist ("..tostring(easing)..")"))
	end

	if not(type(duration) == "number") then
		error(dgsGenAsrt(duration, "dgsAnimTo", 5, "number"))
	end
...
function dgsStopAniming(...)
	...
	if not(dgsIsType(dgsEle) or type(dgsEle) == "number") then
		error(dgsGenAsrt(dgsEle, "dgsStopAniming", 1, "dgs-dxelement/number"))
	end
...
function dgsIsAniming(dgsEle, property)
	if not(dgsIsType(dgsEle)) then
		error(dgsGenAsrt(dgsEle, "dgsIsAniming", 1, "dgs-dxelement"))
	end
	...
	while animIndex <= #animQueue do
		...
		if isElement(dgsEle) then
			...
			if rTick >= startTick then
				if animItem[3] == nil then
					animItem[3] = dgsElementData[dgsEle][ animItem[2] ]
				end
				...
				if reversedProgress then
					rProgress = 1-rProgress
				end
...
function dgsMoveTo(...)
	...
	if not(type(dgsEle) == "table" or dgsIsType(dgsEle)) then
		error(dgsGenAsrt(dgsEle, "dgsMoveTo", 1, "dgs-dxelement/table"))
	end

	if not(type(x) == "number") then
		error(dgsGenAsrt(x, "dgsMoveTo", 2, "number"))
	end

	if not(type(y) == "number") then
		error(dgsGenAsrt(y, "dgsMoveTo", 3, "number"))
	end

	easing = easing or "Linear"

	if not(dgsEasingFunctionExists(easing)) then
		error(dgsGenAsrt(easing, "dgsMoveTo", 5, "easing function doesn't exist ("..tostring(easing)..")"))
	end

	if not(type(duration) == "number") then
		error(dgsGenAsrt(duration, "dgsMoveTo", 6, "number"))
	end
...
function dgsStopMoving(dgsEle)
	if not(type(dgsEle) == "table" or dgsIsType(dgsEle)) then
		error(dgsGenAsrt(dgsEle, "dgsStopMoving", 1, "dgs-dxelement/table"))
	end

	if type(dgsEle) == "table" then
		for i = 1, #dgsEle do
			dgsStopMoving(dgsEle[i])
		end
	else
...
function dgsIsMoving(dgsEle)
	if not(dgsIsType(dgsEle)) then
		error(dgsGenAsrt(dgsEle, "dgsIsMoving", 1, "dgs-dxelement"))
	end
...
function dgsSizeTo(...)
	...
	if not(type(dgsEle) == "table" or dgsIsType(dgsEle)) then
		error(dgsGenAsrt(dgsEle, "dgsSizeTo", 1, "dgs-dxelement/table"))
	end

	if not(type(w) == "number") then
		error(dgsGenAsrt(w, "dgsSizeTo", 2, "number"))
	end

	if not(type(h) == "number") then
		error(dgsGenAsrt(h, "dgsSizeTo", 3, "number"))
	end

	if relative then
		if w > 10 or w < -10 then
			error(dgsGenAsrt(w, "dgsSizeTo", 2, "float between [0, 1]"))
		end

		if h > 10 or h < -10 then
			error(dgsGenAsrt(h, "dgsSizeTo", 3, "float between [0, 1]"))
		end
	end
	...
	if not(dgsEasingFunctionExists(easing)) then
		error(dgsGenAsrt(easing, "dgsSizeTo", 5, "easing function doesn't exist ("..tostring(easing)..")"))
	end

	if not(type(duration) == "number") then
		error(dgsGenAsrt(duration, "dgsSizeTo", 6, "number"))
	end
...
function dgsStopSizing(dgsEle)
	if not(type(dgsEle) == "table" or dgsIsType(dgsEle)) then
		error(dgsGenAsrt(dgsEle, "dgsStopSizing", 1, "dgs-dxelement/table"))
	end
...
function dgsIsSizing(dgsEle)
	if not(dgsIsType(dgsEle)) then
		error(dgsGenAsrt(dgsEle, "dgsIsSizing", 1, "dgs-dxelement"))
	end
...
function dgsAlphaTo(...)
	...
	if not(type(dgsEle) == "table" or dgsIsType(dgsEle)) then
		error(dgsGenAsrt(dgsEle, "dgsAlphaTo", 1, "dgs-dxelement/table"))
	end

	if not(type(alpha) == "number") then
		error(dgsGenAsrt(alpha, "dgsAlphaTo", 2, "number"))
	end

	easing = easing or "Linear"

	if not(dgsEasingFunctionExists(easing)) then
		error(dgsGenAsrt(easing, "dgsAlphaTo", 3, _, "easing function doesn't exist ("..tostring(easing)..")"))
	end

	if not(type(duration) == "number") then
		error(dgsGenAsrt(duration, "dgsAlphaTo", 4, "number"))
	end
...
function dgsStopAlphaing(dgsEle)
	if not(type(dgsEle) == "table" or dgsIsType(dgsEle)) then
		error(dgsGenAsrt(dgsEle, "dgsStopAlphaing", 1, "dgs-dxelement/table"))
	end
...
function dgsIsAlphaing(dgsEle)
	if not(dgsIsType(dgsEle)) then
		error(dgsGenAsrt(dgsEle, "dgsIsAlphaing", 1, "dgs-dxelement"))
	end
