...
function dgsGetElementPositionOnScreen(dgsEle, relativeTo)
	if relativeTo == dgsEle then
		return 0, 0
	end

	if relativeTo == true then
		relativeTo = "screen"
	end
	...
	for i = #parentList, 0, -1 do
		...
		if not absPos then
			...
			absX, absY = absPos[1], absPos[2]
		end

		if not absSize then
			absW, absH = 0, 0
		else
			absW, absH = absSize[1], absSize[2]
		end
		...
		if eleTypeP == "dgs-dxwindow" and eleData.ignoreParentTitle then
			OffsetY = 0
		end
...
function dgsGetPosition(dgsEle, relative, relativeTo)
	if not(dgsIsType(dgsEle)) then
		error(dgsGenAsrt(dgsEle, "dgsGetPosition", 1, "dgs-dxelement"))
	end

	if relativeTo then
		local posX, posY, startElement, brokenElement = dgsGetElementPositionOnScreen(dgsEle, relativeTo)

		if brokenElement then
			error("Bad argument @'dgsGetPosition', Found an infinite loop under "..tostring(brokenElement).."("..dgsGetType(brokenElement).."), start from element "..tostring(startElement).."("..dgsGetType(startElement)..")")
		end
...
function dgsSetPosition(dgsEle, x, y, relative, ...)
	if (x and type(x) ~= "number") then
		error(dgsGenAsrt(x, "dgsSetPosition", 2, "nil/number"))
	end

	if (y and type(y) ~= "number") then
		error(dgsGenAsrt(y, "dgsSetPosition", 3, "nil/number"))
	end

	if type(dgsEle) == "table" then
		for index, dgsEleItem in pairs(dgsEle) do
			if not(dgsIsType(dgsEleItem)) then
				error(dgsGenAsrt(dgsEleItem, "dgsSetPosition", 1, "dgs-dxelement", _, _, "at table index "..i))
			end
		...
		if not(dgsIsType(dgsEle)) then
			error(dgsGenAsrt(dgsEle, "dgsSetPosition", 1, "dgs-dxelement"))
		end
		...
		if select("#", ...) == 2 or pivot then
			...
			if (type(pivotX) ~= "number") then
				error(dgsGenAsrt(pivotX, "dgsSetPosition", 5, "number"))
			end

			if (type(pivotY) ~= "number") then
				error(dgsGenAsrt(pivotY, "dgsSetPosition", 6, "number"))
			end
...
function dgsCenterElement(dgsEle, remainX, remainY)
	if type(dgsEle) == "table" then
		for index, dgsEleItem in pairs(dgsEle) do
			if not(dgsIsType(dgsEleItem)) then
				error(dgsGenAsrt(dgsEleItem, "dgsCenterElement", 1, "dgs-dxelement", _, _, "at table index "..i))
			end
		...
		if not(dgsIsType(dgsEle)) then
			error(dgsGenAsrt(dgsEle, "dgsCenterElement", 1, "dgs-dxelement"))
		end
...
function dgsGetSize(dgsEle, relative)
	if not(dgsIsType(dgsEle)) then
		error(dgsGenAsrt(dgsEle, "dgsGetSize", 1, "dgs-dxelement"))
	end
...
function dgsSetSize(dgsEle, w, h, relative, ...)
	if (w and type(w) ~= "number") then
		error(dgsGenAsrt(w, "dgsSetSize", 2, "nil/number"))
	end

	if (h and type(h) ~= "number") then
		error(dgsGenAsrt(h, "dgsSetSize", 3, "nil/number"))
	end

	if relative then
		if w and (w > 10 or w < -10) then
			error(dgsGenAsrt(w, "dgsSetSize", 2, "float between [0, 1]"))
		end

		if h and (h > 10 or h < -10) then
			error(dgsGenAsrt(h, "dgsSetSize", 3, "float between [0, 1]"))
		end
	end
	...
	if type(dgsEle) == "table" then
		for index, dgsEleItem in pairs(dgsEle) do
			if not(dgsIsType(dgsEleItem)) then
				error(dgsGenAsrt(dgsEleItem, "dgsSetSize", 1, "dgs-dxelement", _, _, "at table index "..i))
			end
		...
		if select("#", ...) == 2 or pivot then
			...
			if (type(pivotX) ~= "number") then
				error(dgsGenAsrt(pivotX, "dgsSetSize", 5, "number"))
			end

			if (type(pivotY) ~= "number") then
				error(dgsGenAsrt(pivotY, "dgsSetSize", 6, "number"))
			end
...
function dgsAttachElements(dgsEle, attachTo, offsetX, offsetY, offsetW, offsetH, relativePos, relativeSize)
	if not(dgsIsType(dgsEle)) then
		error(dgsGenAsrt(dgsEle, "dgsAttachElements", 1, "dgs-dxelement"))
	end

	if dgsGetParent(dgsEle) then
		error(dgsGenAsrt(dgsEle, "dgsAttachElements", 1, _, _, "source dgs element shouldn't have a parent"))
	end

	if not(dgsIsType(attachTo)) then
		error(dgsGenAsrt(attachTo, "dgsAttachElements", 2, "dgs-dxelement"))
	end
...
function dgsElementIsAttached(dgsEle)
	if not(dgsIsType(dgsEle)) then
		error(dgsGenAsrt(dgsEle, "dgsElementIsAttached", 1, "dgs-dxelement"))
	end
...
function dgsDetachElements(dgsEle)
	if not(dgsIsType(dgsEle)) then
		error(dgsGenAsrt(dgsEle, "dgsDetachElements", 1, "dgs-dxelement"))
	end
...
function dgsApplyDetectArea(dgsEle, da)
	if not(dgsIsType(dgsEle)) then
		error(dgsGenAsrt(dgsEle, "dgsApplyDetectArea", 1, "dgs-dxelement"))
	end

	if not(dgsGetType(da) == "dgs-dxdetectarea") then
		error(dgsGenAsrt(da, "dgsApplyDetectArea", 2, "dgs-dxdetectarea"))
	end
...
function dgsRemoveDetectArea(dgsEle)
	if not(dgsIsType(dgsEle)) then
		error(dgsGenAsrt(dgsEle, "dgsRemoveDetectArea", 1, "dgs-dxelement"))
	end
...
function dgsGetDetectArea(dgsEle)
	if not(dgsIsType(dgsEle)) then
		error(dgsGenAsrt(dgsEle, "dgsGetDetectArea", 1, "dgs-dxelement"))
	end
...
function dgsSetEnabled(dgsEle, enabled)
	...
	if type(dgsEle) == "table" then
		for index, dgsEleItem in pairs(dgsEle) do
			if not(dgsIsType(dgsEleItem)) then
				error(dgsGenAsrt(dgsEleItem, "dgsSetEnabled", 1, "dgs-dxelement", _, _, "at table index "..i))
			end
		...
		if not(dgsIsType(dgsEle)) then
			error(dgsGenAsrt(dgsEle, "dgsSetEnabled", 1, "dgs-dxelement"))
		end

		local originalEnabled = dgsElementData[dgsEle].enabled

		if enabled == originalEnabled then
			return true
		end
		...
		if parent then
			parentInherited = dgsElementData[parent].enabledInherited and dgsElementData[parent].enabled
		end
...
function dgsGetEnabled(dgsEle, selfOnly)
	if not(dgsIsType(dgsEle)) then
		error(dgsGenAsrt(dgsEle, "dgsGetEnabled", 1, "dgs-dxelement"))
	end
...
function dgsApplyVisibleInherited(parent, visible)
	...
	for i = 1, #children do
		...
		if dgsOnVisibilityChange[eleType] then
			dgsOnVisibilityChange[eleType](child)
		end

		if not visible and MouseData.focused == child then
			dgsBlur(child)
		end
...
function dgsSetVisible(dgsEle, visible)
	...
	if type(dgsEle) == "table" then
		...
		for index, dgsEleItem in pairs(dgsEle) do
			if not(dgsIsType(dgsEleItem)) then
				error(dgsGenAsrt(dgsEleItem, "dgsSetVisible", 1, "dgs-dxelement", _, _, "at table index "..i))
			end
		...
		if not(dgsIsType(dgsEle)) then
			error(dgsGenAsrt(dgsEle, "dgsSetVisible", 1, "dgs-dxelement"))
		end
		...
		if parent then
			parentInherited = dgsElementData[parent].visibleInherited  and dgsElementData[parent].visible
		end
		...
		if dgsOnVisibilityChange[eleType] then
			dgsOnVisibilityChange[eleType](dgsEle)
		end

		if not visible and MouseData.focused == dgsEle then
			dgsBlur(dgsEle)
		end
...
function dgsGetVisible(dgsEle, selfOnly)
	if not(dgsIsType(dgsEle)) then
		error(dgsGenAsrt(dgsEle, "dgsGetVisible", 1, "dgs-dxelement"))
	end
...
function dgsSetPositionAlignment(dgsEle, horizontal, vertical)
	if not(dgsIsType(dgsEle)) then
		error(dgsGenAsrt(dgsEle, "dgsSetPositionAlignment", 1, "dgs-dxelement"))
	end
...
function dgsGetPositionAlignment(dgsEle)
	if not(dgsIsType(dgsEle)) then
		error(dgsGenAsrt(dgsEle, "dgsGetPositionAlignment", 1, "dgs-dxelement"))
	end
...
function calculateGuiPositionSize(dgsEle, x, y, relativep, sx, sy, relatives, notrigger)
	if not dgsElementData[dgsEle] then
		dgsElementData[dgsEle] = {}
	end
	...
	if x and y then
		if not eleData.absPos then
			eleData.absPos = {}
		end

		if not eleData.rltPos then
			eleData.rltPos = {}
		end
	...
	if sx and sy then
		if not eleData.absSize then
			eleData.absSize = {}
		end

		if not eleData.rltSize then
			eleData.rltSize = {}
		end
...
function dgsSetAlpha(dgsEle, alpha, absolute)
	if not(type(alpha) == "number") then
		error(dgsGenAsrt(alpha, "dgsSetAlpha", 2, "number"))
	end

	if type(dgsEle) == "table" then
		for index, dgsEleItem in pairs(dgsEle) do
			if not(dgsIsType(dgsEleItem)) then
				error(dgsGenAsrt(dgsEleItem, "dgsSetAlpha", 1, "dgs-dxelement", _, _, "at table index "..i))
			end
		...
		if not(dgsIsType(dgsEle)) then
			error(dgsGenAsrt(dgsEle, "dgsSetAlpha", 1, "dgs-dxelement"))
		end
...
function dgsGetAlpha(dgsEle, absolute, includeParent)
	if not(dgsIsType(dgsEle)) then
		error(dgsGenAsrt(dgsEle, "dgsGetAlpha", 1, "dgs-dxelement"))
	end
...
function dgsCreateFont(path, size, bold, quality)
	if not(type(path) == "string") then
		error(dgsGenAsrt(path, "dgsCreateFont", 1, "string"))
	end
	...
	if not fileExists(path) then
		error(dgsGenAsrt(path, "dgsCreateFont", 1, _, _, _, "Couldn't find such file '"..path.."'"))
	end
...
function dgsSetFont(dgsEle, font)
	if type(dgsEle) == "table" then
		for index, dgsEleItem in pairs(dgsEle) do
			if not(dgsIsType(dgsEleItem)) then
				error(dgsGenAsrt(dgsEleItem, "dgsSetFont", 1, "dgs-dxelement", _, _, "at table index "..i))
			end
		...
		if not(dgsIsType(dgsEle)) then
			error(dgsGenAsrt(dgsEle, "dgsSetFont", 1, "dgs-dxelement"))
		end

		local fontType = dgsGetType(font)

		if fontType == "string" then
			if not(fontBuiltIn[font]) then
				error(dgsGenAsrt(font, "dgsSetFont", 2, _, _, _, "font "..font.." doesn't exist"))
			end
...
function dgsGetFont(dgsEle)
	if not(dgsIsType(dgsEle)) then
		error(dgsGenAsrt(dgsEle, "dgsGetFont", 1, "dgs-dxelement"))
	end
...
function dgsSetSystemFont(font, size, bold, quality, styleName, sres)
	if not(type(font) == "string") then
		error(dgsGenAsrt(font, "dgsSetSystemFont", 1, "string"))
	end
	...
	if fontBuiltIn[font] then
		...
		if not fileExists(path) then
			error(dgsGenAsrt(path, "dgsSetSystemFont", 1, _, _, _, "Couldn't find such file '"..path.."'"))
		end
...
function dgsSetText(dgsEle, text)
	if type(dgsEle) == "table" then
		for index, dgsEleItem in pairs(dgsEle) do
			if not(dgsIsType(dgsEleItem)) then
				error(dgsGenAsrt(dgsEleItem, "dgsSetText", 1, "dgs-dxelement", _, _, "at table index "..i))
			end
		...
		if not(dgsIsType(dgsEle)) then
			error(dgsGenAsrt(dgsEle, "dgsSetText", 1, "dgs-dxelement"))
		end
...
function dgsGetText(dgsEle)
	if not(dgsIsType(dgsEle)) then
		error(dgsGenAsrt(dgsEle, "dgsGetText", 1, "dgs-dxelement"))
	end
...
function dgsSetClickingSound(dgsEle, soundPath, volume, button, state)
	if not(dgsIsType(dgsEle)) then
		error(dgsGenAsrt(dgsEle, "dgsSetClickingSound", 1, "dgs-dxelement"))
	end

	if not(type(soundPath) == "string") then
		error(dgsGenAsrt(soundPath, "dgsSetClickingSound", 2, "string"))
	end
	...
	if not fileExists(soundPath) then
		error(dgsGenAsrt(soundPath, "dgsSetClickingSound", 2, _, _, _, "Couldn't find such file '"..soundPath.."'"))
	end
	...
	if not eleData.clickingSound then
		eleData.clickingSound = {}
	end

	if not eleData.clickingSound[button] then
		eleData.clickingSound[button] = {}
	end

	eleData.clickingSound[button][state] = soundPath

	if not eleData.clickingSoundVolume then
		eleData.clickingSoundVolume = {}
	end

	if not eleData.clickingSoundVolume[button] then
		eleData.clickingSoundVolume[button] = {}
	end
...
function dgsSetClickingSoundVolume(dgsEle, volume, button, state)
	if not(dgsIsType(dgsEle)) then
		error(dgsGenAsrt(dgsEle, "dgsSetClickingSoundVolume", 1, "dgs-dxelement"))
	end

	if type(volume) ~= "number" then
		error(dgsGenAsrt(volume, "dgsSetClickingSoundVolume", 2, "number"))
	end
...
function dgsSetPostGUI(dgsEle, state)
	if not(dgsIsType(dgsEle)) then
		error(dgsGenAsrt(dgsEle, "dgsSetPostGUI", 1, "dgs-dxelement"))
	end
...
function dgsGetPostGUI(dgsEle)
	if not(dgsIsType(dgsEle)) then
		error(dgsGenAsrt(dgsEle, "dgsGetPostGUI", 1, "dgs-dxelement"))
	end
...
function dgsGetMouseEnterGUI()
	return MouseData.entered
end

function dgsGetMouseLeaveGUI()
	return MouseData.left
end

function dgsGetFocusedGUI()
	return MouseData.focused
end
...
function dgsFocus(dgsEle)
	if not(dgsIsType(dgsEle)) then
		error(dgsGenAsrt(dgsEle, "dgsFocus", 1, "dgs-dxelement"))
	end
...
function dgsGetCursorPosition(rltEle, rlt, forceOnScreen)
	if dgsGetCursorVisible() then
		if rltEle and dgsIsType(rltEle) then	--relative to element has higher priority
			...
			while(parent) do
				if dgsGetType(parent) == "dgs-dx3dinterface" then
					has3DInterface = parent
					break
				end
...
function dgsGetMultiClickInterval()
	return multiClick.Interval
end

function dgsSetMultiClickInterval(interval)
	if not(type(interval) == "number") then
		error(dgsGenAsrt(interval, "dgsSetClickInterval", 1, "number"))
	end
...
--Mouse Stay Delay
function dgsGetMouseStayDelay()
	return mouseStay.delay
end

function dgsSetMouseStayDelay(delay)
	if not(type(delay) == "number") then
		error(dgsGenAsrt(delay, "dgsSetMouseStayDelay", 1, "number"))
	end
...
function dgsAddMoveHandler(dgsEle, x, y, w, h, xRel, yRel, wRel, hRel, forceReplace)
	if not(dgsIsType(dgsEle)) then
		error(dgsGenAsrt(dgsEle, "dgsAddMoveHandler", 1, "dgs-dxelement"))
	end
...
function dgsRemoveMoveHandler(dgsEle)
	if not(dgsIsType(dgsEle)) then
		error(dgsGenAsrt(dgsEle, "dgsRemoveMoveHandler", 1, "dgs-dxelement"))
	end
...
function dgsIsMoveHandled(dgsEle)
	if not(dgsIsType(dgsEle)) then
		error(dgsGenAsrt(dgsEle, "dgsIsMoveHandled", 1, "dgs-dxelement"))
	end
...
function dgsAddSizeHandler(dgsEle, left, right, top, bottom, leftRel, rightRel, topRel, bottomRel, forceReplace)
	if not(dgsIsType(dgsEle)) then
		error(dgsGenAsrt(dgsEle, "dgsAddSizeHandler", 1, "dgs-dxelement"))
	end
...
function dgsRemoveSizeHandler(dgsEle)
	if not(dgsIsType(dgsEle)) then
		error(dgsGenAsrt(dgsEle, "dgsRemoveSizeHandler", 1, "dgs-dxelement"))
	end
...
function dgsIsSizeHandled(dgsEle)
	if not(dgsIsType(dgsEle)) then
		error(dgsGenAsrt(dgsEle, "dgsIsSizeHandled", 1, "dgs-dxelement"))
	end
...
function dgsAttachToAutoDestroy(element, dgsEle, index)
	...
	if not(dgsIsType(dgsEle)) then
		error(dgsGenAsrt(dgsEle, "dgsAttachToAutoDestroy", 2, "dgs-dxelement"))
	end
...
function dgsDetachFromAutoDestroy(element, dgsEle)
	if not(isElement(element)) then
		error(dgsGenAsrt(element, "dgsDetachFromAutoDestroy", 1, "element"))
	end

	if not(dgsIsType(dgsEle)) then
		error(dgsGenAsrt(dgsEle, "dgsDetachFromAutoDestroy", 2, "dgs-dxelement"))
	end
...
function dgsTranslationTableExists(name)
	if not(type(name) == "string") then
		error(dgsGenAsrt(name, "dgsTranslationTableExists", 1, "string"))
	end
...
function dgsSetTranslationTable(name, tab)
	if not(type(name) == "string") then
		error(dgsGenAsrt(name, "dgsSetTranslationTable", 1, "string"))
	end

	if not(not table or type(tab) == "table") then
		error(dgsGenAsrt(tab, "dgsSetTranslationTable", 1, "table/nil"))
	end

	if tab then
		LanguageTranslation[name] = tab

		if not LanguageTranslationAttach[name] then
			LanguageTranslationAttach[name] = {}
		end
...
function dgsAttachToTranslation(dgsEle, name)
	if not(dgsIsType(dgsEle)) then
		error(dgsGenAsrt(dgsEle, "dgsAttachToTranslation", 1, "dgs-dxelement"))
	end
...
function dgsDetachFromTranslation(dgsEle)
	if not(dgsIsType(dgsEle)) then
		error(dgsGenAsrt(dgsEle, "dgsDetachFromTranslation", 1, "dgs-dxelement"))
	end
...
function dgsGetTranslationName(dgsEle)
	if not(dgsIsType(dgsEle)) then
		error(dgsGenAsrt(dgsEle, "dgsGetTranslationName", 1, "dgs-dxelement"))
	end
...
function dgsTranslateText(textTable, translationTableName)
	if type(textTable) == "table" then
		...
		if type(value) == "table" then	--Conditional Translation
			...
			if #value%2 == 0 then
				error("Bad argument @dgsTranslateText at argument 1, Bad conditional translation item count (should be odd, got even), maybe missing default translation")
			end
			...
			for i = 1, #value-1, 2 do --Odd item is conditional, Even item is translation, and skip default translation
				if not cTranslationBuffer[value[i]] then
					...
					if not fnc then
						error("Bad argument @dgsTranslateText, failed to load conditional ("..err..") at dictionary:"..translation.."[\""..textTable[1].."\"]["..math.floor(i/2).."]")
					end
...
function dgsGetTranslationFont(dgsEle, fontTable, sourceResource)
	if not(dgsIsType(dgsEle)) then
		error(dgsGenAsrt(dgsEle, "dgsTranslate", 1, "dgs-dxelement"))
	end
...
function dgsApplyLanguageChange(name, translation, attach)
	for i = 1, #attach do
		...
		if isElement(dgsEle) then
			...
			if dgsOnTranslationUpdate[dgsType] then
				dgsOnTranslationUpdate[dgsType](dgsEle)
			else
				dgsOnTranslationUpdate.default(dgsEle)
			end
		end
	end
end
...
function dgsTranslate(dgsEle, textTable, sourceResource, skipPropertyListener)
	...
	if type(textTable) == "table" then
		...
		if type(value) == "table" then	--Conditional Translation
			...
			if #value%2 == 0 then
				error(dgsGenAsrt(dgsEle, "dgsTranslate", _, _, _, "Bad conditional translation item count (should be odd, got even), maybe missing default translation"))
			end
			...
			for i = 1, #value-1, 2 do --Odd item is conditional, Even item is translation, and skip default translation
				if not cTranslationBuffer[value[i]] then
					...
					if not fnc then
						error("Bad argument @dgsTranslate, failed to load conditional ("..err..") at dictionary:"..translation.."[\""..textTable[1].."\"]["..math.floor(i/2).."]")
					end
...
function dgs3DSetPosition(ele3D, x, y, z)
	if not(dgsIsType(ele3D, "dgsType3D")) then
		error(dgsGenAsrt(ele3D, "dgs3DSetPosition", 1, "dgs-dx3delement"))
	end

	if not(type(x) == "number") then
		error(dgsGenAsrt(x, "dgs3DSetPosition", 2, "number"))
	end

	if not(type(y) == "number") then
		error(dgsGenAsrt(y, "dgs3DSetPosition", 3, "number"))
	end

	if not(type(z) == "number") then
		error(dgsGenAsrt(z, "dgs3DSetPosition", 4, "number"))
	end
...
function dgs3DGetPosition(ele3D)
	if not(dgsIsType(ele3D, "dgsType3D")) then
		error(dgsGenAsrt(ele3D, "dgs3DGetPosition", 1, "dgs-dx3delement"))
	end
...
function dgs3DSetInterior(ele3D, interior)
	if not(dgsIsType(ele3D, "dgsType3D")) then
		error(dgsGenAsrt(ele3D, "dgs3DSetInterior", 1, "dgs-dx3delement"))
	end

	local inRange = interior >= -1

	if not(type(interior) == "number" and inRange) then
		error(dgsGenAsrt(interior, "dgs3DSetInterior", 2, "number", "-1~+∞", inRange and "Out Of Range"))
	end
...
function dgs3DGetInterior(ele3D)
	if not(dgsIsType(ele3D, "dgsType3D")) then
		error(dgsGenAsrt(ele3D, "dgs3DGetInterior", 1, "dgs-dx3delement"))
	end
...
function dgs3DSetDimension(ele3D, dimension)
	if not(dgsIsType(ele3D, "dgsType3D")) then
		error(dgsGenAsrt(ele3D, "dgs3DSetDimension", 1, "dgs-dx3delement"))
	end

	local inRange = dimension >= -1 and dimension <= 65535

	if not(type(dimension) == "number" and dimension) then
		error(dgsGenAsrt(dimension, "dgs3DSetDimension", 2, "number", "-1~+∞", inRange and "Out Of Range"))
	end
...
function dgs3DGetDimension()
	if not(dgsIsType(ele3D, "dgsType3D")) then
		error(dgsGenAsrt(ele3D, "dgs3DGetDimension", 1, "dgs-dx3delement"))
	end
