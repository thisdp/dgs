...
function dgsCreateLine(...)
	...
	if not(type(x) == "number") then
		error(dgsGenAsrt(x, "dgsCreateLine", 1, "number"))
	end

	if not(type(y) == "number") then
		error(dgsGenAsrt(y, "dgsCreateLine", 2, "number"))
	end

	if not(type(w) == "number") then
		error(dgsGenAsrt(w, "dgsCreateLine", 3, "number"))
	end

	if not(type(h) == "number") then
		error(dgsGenAsrt(h, "dgsCreateLine", 4, "number"))
	end
...
function dgsLineAddItem(line, sx, sy, ex, ey, width, color, isRelative)
	if not(dgsGetType(line) == "dgs-dxline") then
		error(dgsGenAsrt(line, "dgsLineAddItem", 1, "dgs-dxline"))
	end

	if sx or sy then
		if not(type(sx) == "number") then
			error(dgsGenAsrt(sx, "dgsLineAddItem", 2, "number"))
		end

		if not(type(sy) == "number") then
			error(dgsGenAsrt(sy, "dgsLineAddItem", 3, "number"))
		end
	end

	if not(type(ex) == "number") then
		error(dgsGenAsrt(ex, "dgsLineAddItem", 4, "number"))
	end

	if not(type(ey) == "number") then
		error(dgsGenAsrt(ey, "dgsLineAddItem", 5, "number"))
	end
...
function dgsLineRemoveItem(line, index)
	if not(dgsGetType(line) == "dgs-dxline") then
		error(dgsGenAsrt(line, "dgsLineRemoveItem", 1, "dgs-dxline"))
	end
	...
	if not(type(index) == "number" and inRange) then
		error(dgsGenAsrt(index, "dgsLineRemoveItem", 2, "number", "1~"..(#lData), inRange and "Out Of Range"))
	end
...
function dgsLineSetItemPosition(line, index, sx, sy, ex, ey, isRelative)
	if not(dgsGetType(line) == "dgs-dxline") then
		error(dgsGenAsrt(line, "dgsLineSetItemPosition", 1, "dgs-dxline"))
	end
	...
	if not(type(index) == "number" and inRange) then
		error(dgsGenAsrt(index, "dgsLineSetItemPosition", 2, "number", "1~"..(#lData), inRange and "Out Of Range"))
	end
	...
	if sx or sy then
		if not(type(sx) == "number") then
			error(dgsGenAsrt(sx, "dgsLineSetItemPosition", 3, "number"))
		end

		if not(type(sy) == "number") then
			error(dgsGenAsrt(sy, "dgsLineSetItemPosition", 4, "number"))
		end
	end

	if not(type(ex) == "number") then
		error(dgsGenAsrt(ex, "dgsLineSetItemPosition", 5, "number"))
	end

	if not(type(ey) == "number") then
		error(dgsGenAsrt(ey, "dgsLineSetItemPosition", 6, "number"))
	end
...
function dgsLineGetItemPosition(line, index)
	if not(dgsGetType(line) == "dgs-dxline") then
		error(dgsGenAsrt(line, "dgsLineGetItemPosition", 1, "dgs-dxline"))
	end
	...
	if not(type(index) == "number" and inRange) then
		error(dgsGenAsrt(index, "dgsLineGetItemPosition", 2, "number", "1~"..(#lData), inRange and "Out Of Range"))
	end
...
function dgsLineSetItemWidth(line, index, width)
	if not(dgsGetType(line) == "dgs-dxline") then
		error(dgsGenAsrt(line, "dgsLineSetItemWidth", 1, "dgs-dxline"))
	end
	...
	if not(type(index) == "number" and inRange) then
		error(dgsGenAsrt(index, "dgsLineSetItemWidth", 2, "number", "1~"..(#lData), inRange and "Out Of Range"))
	end

	if not(not width or type(width) == "number") then
		error(dgsGenAsrt(width, "dgsLineSetItemWidth", 3, "nil/number"))
	end
...
function dgsLineGetItemWidth(line, index)
	if not(dgsGetType(line) == "dgs-dxline") then
		error(dgsGenAsrt(line, "dgsLineGetItemWidth", 1, "dgs-dxline"))
	end
	...
	if not(type(index) == "number" and inRange) then
		error(dgsGenAsrt(index, "dgsLineGetItemWidth", 2, "number", "1~"..(#lData), inRange and "Out Of Range"))
	end
...
function dgsLineSetItemColor(line, index, color)
	if not(dgsGetType(line) == "dgs-dxline") then
		error(dgsGenAsrt(line, "dgsLineSetItemColor", 1, "dgs-dxline"))
	end
	...
	if not(type(index) == "number" and inRange) then
		error(dgsGenAsrt(index, "dgsLineSetItemColor", 2, "number", "1~"..(#lData), inRange and "Out Of Range"))
	end

	if not(not color or type(color) == "number") then
		error(dgsGenAsrt(color, "dgsLineSetItemColor", 3, "nil/number"))
	end
...
function dgsLineGetItemColor(line, index)
	if not(dgsGetType(line) == "dgs-dxline") then
		error(dgsGenAsrt(line, "dgsLineGetItemColor", 1, "dgs-dxline"))
	end
	...
	if not(type(index) == "number" and inRange) then
		error(dgsGenAsrt(index, "dgsLineGetItemColor", 2, "number", "1~"..(#lData), inRange and "Out Of Range"))
	end
