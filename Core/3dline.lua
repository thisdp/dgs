...
function dgsCreate3DLine(...)
	...
	if not(type(x) == "number") then
		error(dgsGenAsrt(x, "dgsCreate3DLine", 1, "number"))
	end

	if not(type(y) == "number") then
		error(dgsGenAsrt(y, "dgsCreate3DLine", 2, "number"))
	end

	if not(type(z) == "number") then
		error(dgsGenAsrt(z, "dgsCreate3DLine", 3, "number"))
	end
...
function dgs3DLineSetLineType(line, mode)
	if not(dgsGetType(line) == "dgs-dx3dline") then
		error(dgsGenAsrt(line, "dgs3DLineSetLineType", 1, "dgs-dx3dline"))
	end
...
function dgs3DLineGetLineType(line)
	if not(dgsGetType(line) == "dgs-dx3dline") then
		error(dgsGenAsrt(line, "dgs3DLineGetLineType", 1, "dgs-dx3dline"))
	end
...
function dgs3DLineAddItem(line, sx, sy, sz, ex, ey, ez, width, color, isRelative)
	if not(dgsGetType(line) == "dgs-dx3dline") then
		error(dgsGenAsrt(line, "dgs3DLineAddItem", 1, "dgs-dx3dline"))
	end

	if sx or sy or sz then
		if not(type(sx) == "number") then
			error(dgsGenAsrt(sx, "dgs3DLineAddItem", 2, "number"))
		end

		if not(type(sy) == "number") then
			error(dgsGenAsrt(sy, "dgs3DLineAddItem", 3, "number"))
		end

		if not(type(sz) == "number") then
			error(dgsGenAsrt(sz, "dgs3DLineAddItem", 4, "number"))
		end
	end

	if not(type(ex) == "number") then
		error(dgsGenAsrt(ex, "dgs3DLineAddItem", 5, "number"))
	end

	if not(type(ey) == "number") then
		error(dgsGenAsrt(ey, "dgs3DLineAddItem", 6, "number"))
	end

	if not(type(ez) == "number") then
		error(dgsGenAsrt(ez, "dgs3DLineAddItem", 7, "number"))
	end
...
function dgs3DLineRemoveItem(line, index)
	if not(dgsGetType(line) == "dgs-dx3dline") then
		error(dgsGenAsrt(line, "dgs3DLineRemoveItem", 1, "dgs-dx3dline"))
	end

	local lData = dgsElementData[line].lineData
	local inRange = index >= 1 and index <= #lData

	if not(type(index) == "number" and inRange) then
		error(dgsGenAsrt(index, "dgs3DLineRemoveItem", 2, "number", "1~"..(#lData), inRange and "Out Of Range"))
	end
...
function dgs3DLineSetItemPosition(line, index, sx, sy, sz, ex, ey, ez)
	if not(dgsGetType(line) == "dgs-dx3dline") then
		error(dgsGenAsrt(line, "dgs3DLineSetItemPosition", 1, "dgs-dx3dline"))
	end
	...
	if not(type(index) == "number" and inRange) then
		error(dgsGenAsrt(index, "dgs3DLineSetItemPosition", 2, "number", "1~"..(#lData), inRange and "Out Of Range"))
	end
	...
	if sx or sy or sz then
		if not(type(sx) == "number") then
			error(dgsGenAsrt(sx, "dgs3DLineSetItemPosition", 3, "number"))
		end

		if not(type(sy) == "number") then
			error(dgsGenAsrt(sy, "dgs3DLineSetItemPosition", 4, "number"))
		end

		if not(type(sz) == "number") then
			error(dgsGenAsrt(sz, "dgs3DLineSetItemPosition", 5, "number"))
		end
	end

	if not(type(ex) == "number") then
		error(dgsGenAsrt(ex, "dgs3DLineSetItemPosition", 6, "number"))
	end

	if not(type(ey) == "number") then
		error(dgsGenAsrt(ey, "dgs3DLineSetItemPosition", 7, "number"))
	end

	if not(type(ez) == "number") then
		error(dgsGenAsrt(ez, "dgs3DLineSetItemPosition", 8, "number"))
	end
...
function dgs3DLineGetItemPosition(line, index)
	if not(dgsGetType(line) == "dgs-dx3dline") then
		error(dgsGenAsrt(line, "dgs3DLineGetItemPosition", 1, "dgs-dx3dline"))
	end
	...
	if not(type(index) == "number" and inRange) then
		error(dgsGenAsrt(index, "dgs3DLineGetItemPosition", 2, "number", "1~"..(#lData), inRange and "Out Of Range"))
	end
...
function dgs3DLineSetItemWidth(line, index, width)
	if not(dgsGetType(line) == "dgs-dx3dline") then
		error(dgsGenAsrt(line, "dgs3DLineSetItemWidth", 1, "dgs-dx3dline"))
	end
	...
	if not(type(index) == "number" and inRange) then
		error(dgsGenAsrt(index, "dgs3DLineSetItemWidth", 2, "number", "1~"..(#lData), inRange and "Out Of Range"))
	end

	if not(not width or type(width) == "number") then
		error(dgsGenAsrt(width, "dgs3DLineSetItemWidth", 3, "nil/number"))
	end
...
function dgs3DLineGetItemWidth(line, index)
	if not(dgsGetType(line) == "dgs-dx3dline") then
		error(dgsGenAsrt(line, "dgs3DLineGetItemWidth", 1, "dgs-dx3dline"))
	end
	...
	if not(type(index) == "number" and inRange) then
		error(dgsGenAsrt(index, "dgs3DLineGetItemWidth", 2, "number", "1~"..(#lData), inRange and "Out Of Range"))
	end
...
function dgs3DLineSetItemColor(line, index, color)
	if not(dgsGetType(line) == "dgs-dx3dline") then
		error(dgsGenAsrt(line, "dgs3DLineSetItemColor", 1, "dgs-dx3dline"))
	end
	...
	if not(type(index) == "number" and inRange) then
		error(dgsGenAsrt(index, "dgs3DLineSetItemColor", 2, "number", "1~"..(#lData), inRange and "Out Of Range"))
	end

	if not(not color or type(color) == "number") then
		error(dgsGenAsrt(color, "dgs3DLineSetItemColor", 3, "nil/number"))
	end
...
function dgs3DLineGetItemColor(line, index)
	if not(dgsGetType(line) == "dgs-dx3dline") then
		error(dgsGenAsrt(line, "dgs3DLineGetItemColor", 1, "dgs-dx3dline"))
	end
	...
	if not(type(index) == "number" and inRange) then
		error(dgsGenAsrt(index, "dgs3DLineGetItemColor", 2, "number", "1~"..(#lData), inRange and "Out Of Range"))
	end
...
function dgs3DLineAttachToElement(line, element, offX, offY, offZ, offRX, offRY, offRZ)
	if not(dgsGetType(line) == "dgs-dx3dline") then
		error(dgsGenAsrt(line, "dgs3DLineAttachToElement", 1, "dgs-dx3dline"))
	end

	if not(isElement(element)) then
		error(dgsGenAsrt(element, "dgs3DLineAttachToElement", 2, "element"))
	end
...
function dgs3DLineIsAttached(line)
	if not(dgsGetType(line) == "dgs-dx3dline") then
		error(dgsGenAsrt(line, "dgs3DLineIsAttached", 1, "dgs-dx3dline"))
	end
...
function dgs3DLineDetachFromElement(line)
	if not(dgsGetType(line) == "dgs-dx3dline") then
		error(dgsGenAsrt(line, "dgs3DLineDetachFromElement", 1, "dgs-dx3dline"))
	end
...
function dgs3DLineSetAttachedOffsets(line, offX, offY, offZ, offRX, offRY, offRZ)
	if not(dgsGetType(line) == "dgs-dx3dline") then
		error(dgsGenAsrt(line, "dgs3DLineSetAttachedOffsets", 1, "dgs-dx3dline"))
	end
...
function dgs3DLineGetAttachedOffsets(line, offX, offY, offZ, offRX, offRY, offRZ)
	if not(dgsGetType(line) == "dgs-dx3dline") then
		error(dgsGenAsrt(line, "dgs3DLineGetAttachedOffsets", 1, "dgs-dx3dline"))
	end
...
function dgs3DLineSetRotation(line, rx, ry, rz)
	if not(dgsGetType(line) == "dgs-dx3dline") then
		error(dgsGenAsrt(line, "dgs3DLineSetRotation", 1, "dgs-dx3dline"))
	end

	if not(type(rx) == "number") then
		error(dgsGenAsrt(rx, "dgs3DLineSetRotation", 2, "number"))
	end

	if not(type(ry) == "number") then
		error(dgsGenAsrt(ry, "dgs3DLineSetRotation", 3, "number"))
	end

	if not(type(rz) == "number") then
		error(dgsGenAsrt(rz, "dgs3DLineSetRotation", 4, "number"))
	end
...
function dgs3DLineGetRotation(line)
	if not(dgsGetType(line) == "dgs-dx3dline") then
		error(dgsGenAsrt(line, "dgs3DLineGetRotation", 1, "dgs-dx3dline"))
	end
