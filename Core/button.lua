...
function dgsCreateButton(...)
	...
	if not(type(x) == "number") then
		error(dgsGenAsrt(x, "dgsCreateButton", 1, "number"))
	end

	if not(type(y) == "number") then
		error(dgsGenAsrt(y, "dgsCreateButton", 2, "number"))
	end

	if not(type(w) == "number") then
		error(dgsGenAsrt(w, "dgsCreateButton", 3, "number"))
	end

	if not(type(h) == "number") then
		error(dgsGenAsrt(h, "dgsCreateButton", 4, "number"))
	end
	...
	dgsApplyGeneralProperties(button, sRes)
...
function dgsButtonGetTextExtent(button)
	if dgsGetType(button) ~= "dgs-dxbutton" then
		error(dgsGenAsrt(button, "dgsButtonGetTextExtent", 1, "dgs-dxbutton"))
	end
...
function dgsButtonGetFontHeight(button)
	if dgsGetType(button) ~= "dgs-dxbutton" then
		error(dgsGenAsrt(button, "dgsButtonGetFontHeight", 1, "dgs-dxbutton"))
	end
...
function dgsButtonGetTextSize(button)
	if dgsGetType(button) ~= "dgs-dxbutton" then
		error(dgsGenAsrt(button, "dgsButtonGetTextSize", 1, "dgs-dxbutton"))
	end
...
function dgsButtonMakeForm(button, forms)
	if dgsGetType(button) ~= "dgs-dxbutton" then
		error(dgsGenAsrt(button, "dgsButtonMakeForm", 1, "dgs-dxbutton"))
	end

	for key, ele in pairs(forms) do
		local dgsType = dgsGetType(ele)

		if not (dgsType == "dgs-dxedit" or dgsType == "dgs-dxmemo") then
			error(dgsGenAsrt(ele, "dgsButtonMakeForm", 2, "dgs-dxedit/dgs-dxmemo", "at"))
		end
	end
...
function dgsButtonRemoveForm(button)
	if dgsGetType(button) ~= "dgs-dxbutton" then
		error(dgsGenAsrt(button, "dgsButtonRemoveForm", 1, "dgs-dxbutton"))
	end
...
dgsRenderer["dgs-dxbutton"] = function(source, x, y, w, h, mx, my, cx, cy, enabledInherited, enabledSelf, eleData, parentAlpha, isPostGUI, rndtgt)
	...
	if eleData.currentState ~= buttonState then
		...
		if not renderBuffer.startContext then
			renderBuffer.startContext = {}
		end
