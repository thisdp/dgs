...
function dgsCreateBrowser(...)
	...
	if not(type(x) == "number") then
		error(dgsGenAsrt(x, "dgsCreateBrowser", 1, "number"))
	end

	if not(type(y) == "number") then
		error(dgsGenAsrt(y, "dgsCreateBrowser", 2, "number"))
	end

	if not(type(w) == "number") then
		error(dgsGenAsrt(w, "dgsCreateBrowser", 3, "number"))
	end

	if not(type(h) == "number") then
		error(dgsGenAsrt(h, "dgsCreateBrowser", 4, "number"))
	end

	local browser = createBrowser(1, 1, isLocal and true or false, isTransparent and true or false)

	if not isElement(browser) then
		error(dgsGenAsrt(browser, "dgsCreateBrowser", _, _, _, _, "Failed to create remote browser (createBrowser returns false)!"))
	end
	...
	addEventHandler("onDgsFocus", browser, function()
		focusBrowser(source)
	end, false)
	addEventHandler("onDgsBlur", browser, function()
		focusBrowser()
	end, false)
