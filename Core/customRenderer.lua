...
function dgsCreateCustomRenderer(customFnc, width, height, useRT)
	...
	if customFnc then
		...
		dgsSetData(cr, "customRenderer", function()
			return false
		end)
	end
...
function dgsCustomRendererSetRenderTargetResolution(cr, width, height)
	if dgsGetType(cr) ~= "dgs-dxcustomrenderer" then
		error(dgsGenAsrt(cr, "dgsCustomRendererSetRenderTargetResolution", 1, "dgs-dxcustomrenderer"))
	end

	if not(not width or type(width) == "number") then
		error(dgsGenAsrt(width, "dgsCustomRendererSetRenderTargetResolution", 2, "number/nil"))
	end

	if not(not height or type(height) == "number") then
		error(dgsGenAsrt(height, "dgsCustomRendererSetRenderTargetResolution", 3, "number/nil"))
	end

	local eleData = dgsElementData[cr]

	if not eleData.useRenderTarget then
		error(dgsGenAsrt(height, "dgsCustomRendererSetRenderTargetResolution", 1, _, "Render Target is not enabled for this custom renderer"))
	end

	if eleData.renderTargetResolution[1] ~= width or eleData.renderTargetResolution[2] ~= height then
		if isElement(eleData.renderTarget) then
			destroyElement(eleData.renderTarget)
		end
...
function dgsCustomRendererGetRenderTargetResolution(cr)
	if dgsGetType(cr) ~= "dgs-dxcustomrenderer" then
		error(dgsGenAsrt(cr, "dgsCustomRendererGetRenderTargetResolution", 1, "dgs-dxcustomrenderer"))
	end
...
function dgsCustomRendererGetRenderTarget(cr)
	if dgsGetType(cr) ~= "dgs-dxcustomrenderer" then
		error(dgsGenAsrt(cr, "dgsCustomRendererGetRenderTarget", 1, "dgs-dxcustomrenderer"))
	end
...
function dgsCustomRendererSetFunction(cr, fncStr)
	if dgsGetType(cr) ~= "dgs-dxcustomrenderer" then
		error(dgsGenAsrt(cr, "dgsCustomRendererSetFunction", 1, "dgs-dxcustomrenderer"))
	end
	...
	if fncType == "function" then
		...
		if not (type(fncStr) == "string") then
			error(dgsGenAsrt(fncStr, "dgsCustomRendererSetFunction", 2, "string"))
		end
...
..fncStr
		...
		if not fnc then
			error(dgsGenAsrt(fnc, "dgsCustomRendererSetFunction", 2, _, _, _, "Failed to load function:"..err))
		end
...
function dgsCustomRendererSetBackEndFunction(cr, fncStr)
	if dgsGetType(cr) ~= "dgs-dxcustomrenderer" then
		error(dgsGenAsrt(cr, "dgsCustomRendererSetBackEndFunction", 1, "dgs-dxcustomrenderer"))
	end
	...
	if fncType == "function" then
		...
		if not (type(fncStr) == "string") then
			error(dgsGenAsrt(fncStr, "dgsCustomRendererSetBackEndFunction", 2, "string"))
		end
		...
		if not fnc then
			error(dgsGenAsrt(fnc, "dgsCustomRendererSetBackEndFunction", 2, _, _, _, "Failed to load function:"..err))
		end
