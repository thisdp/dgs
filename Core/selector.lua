...
function dgsCreateSelector(...)
	...
	if not(type(x) == "number") then
		error(dgsGenAsrt(x, "dgsCreateSelector", 1, "number"))
	end

	if not(type(y) == "number") then
		error(dgsGenAsrt(y, "dgsCreateSelector", 2, "number"))
	end

	if not(type(w) == "number") then
		error(dgsGenAsrt(w, "dgsCreateSelector", 3, "number"))
	end

	if not(type(h) == "number") then
		error(dgsGenAsrt(h, "dgsCreateSelector", 4, "number"))
	end
...
function dgsSelectorAddItem(selector, text, pos)
	if dgsGetType(selector) ~= "dgs-dxselector" then
		error(dgsGenAsrt(selector, "dgsSelectorAddItem", 1, "dgs-dxselector"))
	end
...
function dgsSelectorRemoveItem(selector, i)
	if dgsGetType(selector) ~= "dgs-dxselector" then
		error(dgsGenAsrt(selector, "dgsSelectorRemoveItem", 1, "dgs-dxselector"))
	end
	...
	if not (iIsNum and not iNInRange) then
		error(dgsGenAsrt(i, "dgsSelectorRemoveItem", 2, "number", "1~"..iLen, iNInRange and "Out Of Range"))
	end
...
function dgsSelectorClear(selector)
	if dgsGetType(selector) ~= "dgs-dxselector" then
		error(dgsGenAsrt(selector, "dgsSelectorClear", 1, "dgs-dxselector"))
	end
...
function dgsSelectorSetSelectedItem(selector, i)
	if dgsGetType(selector) ~= "dgs-dxselector" then
		error(dgsGenAsrt(selector, "dgsSelectorSetSelectedItem", 1, "dgs-dxselector"))
	end

	if not(type(i) == "number") then
		error(dgsGenAsrt(i, "dgsSelectorSetSelectedItem", 2, "number"))
	end
...
function dgsSelectorGetSelectedItem(selector)
	if dgsGetType(selector) ~= "dgs-dxselector" then
		error(dgsGenAsrt(selector, "dgsSelectorGetSelectedItem", 1, "dgs-dxselector"))
	end
...
function dgsSelectorGetItemText(selector, i, retTransOrig)
	if dgsGetType(selector) ~= "dgs-dxselector" then
		error(dgsGenAsrt(selector, "dgsSelectorGetItemText", 1, "dgs-dxselector"))
	end
	...
	if not (iIsNum and not iNInRange) then
		error(dgsGenAsrt(i, "dgsSelectorGetItemText", 2, "number", "1~"..iLen, iNInRange and "Out Of Range"))
	end
...
function dgsSelectorSetItemText(selector, i, text)
	if dgsGetType(selector) ~= "dgs-dxselector" then
		error(dgsGenAsrt(selector, "dgsSelectorSetItemText", 1, "dgs-dxselector"))
	end
	...
	if not (iIsNum and not iNInRange) then
		error(dgsGenAsrt(i, "dgsSelectorSetItemText", 2, "number", "1~"..iLen, iNInRange and "Out Of Range"))
	end
...
function dgsSelectorSetItemData(selector, i, ...)
	if dgsGetType(selector) ~= "dgs-dxselector" then
		error(dgsGenAsrt(selector, "dgsSelectorSetItemData", 1, "dgs-dxselector"))
	end
	...
	if not (iIsNum and not iNInRange) then
		error(dgsGenAsrt(i, "dgsSelectorSetItemData", 2, "number", "1~"..iLen, iNInRange and "Out Of Range"))
	end
...
function dgsSelectorGetItemData(selector, i, ...)
	if dgsGetType(selector) ~= "dgs-dxselector" then
		error(dgsGenAsrt(selector, "dgsSelectorGetItemData", 1, "dgs-dxselector"))
	end
	...
	if not (iIsNum and not iNInRange) then
		error(dgsGenAsrt(i, "dgsSelectorGetItemData", 2, "number", "1~"..iLen, iNInRange and "Out Of Range"))
	end
...
function dgsSelectorSetItemColor(selector, i, color)
	if dgsGetType(selector) ~= "dgs-dxselector" then
		error(dgsGenAsrt(selector, "dgsSelectorSetItemColor", 1, "dgs-dxselector"))
	end
	...
	if not (iIsNum and not iNInRange) then
		error(dgsGenAsrt(i, "dgsSelectorSetItemColor", 2, "number", "1~"..iLen, iNInRange and "Out Of Range"))
	end

	if not (type(color) == "number") then
		error(dgsGenAsrt(args[3], "dgsSelectorSetItemColor", 3, "number"))
	end
...
function dgsSelectorGetItemColor(selector, i, color)
	if dgsGetType(selector) ~= "dgs-dxselector" then
		error(dgsGenAsrt(selector, "dgsSelectorGetItemColor", 1, "dgs-dxselector"))
	end
	...
	if not (iIsNum and not iNInRange) then
		error(dgsGenAsrt(i, "dgsSelectorGetItemColor", 2, "number", "1~"..iLen, iNInRange and "Out Of Range"))
	end
...
function dgsSelectorSetItemFont(selector, i, font)
	if dgsGetType(selector) ~= "dgs-dxselector" then
		error(dgsGenAsrt(selector, "dgsSelectorSetItemFont", 1, "dgs-dxselector"))
	end
	...
	if not (iIsNum and not iNInRange) then
		error(dgsGenAsrt(i, "dgsSelectorSetItemFont", 2, "number", "1~"..iLen, iNInRange and "Out Of Range"))
	end

	if not (fontBuiltIn[font] or dgsGetType(font) == "dx-font") then
		error(dgsGenAsrt(font, "dgsSelectorSetItemFont", 3, "dx-font/string", _, "invalid font"))
	end
...
function dgsSelectorGetItemFont(selector, i, font)
	if dgsGetType(selector) ~= "dgs-dxselector" then
		error(dgsGenAsrt(selector, "dgsSelectorGetItemFont", 1, "dgs-dxselector"))
	end
	...
	if not (iIsNum and not iNInRange) then
		error(dgsGenAsrt(i, "dgsSelectorGetItemFont", 2, "number", "1~"..iLen, iNInRange and "Out Of Range"))
	end
...
function dgsSelectorSetItemTextSize(selector, i, sizeX, sizeY)
	if dgsGetType(selector) ~= "dgs-dxselector" then
		error(dgsGenAsrt(selector, "dgsSelectorSetItemTextSize", 1, "dgs-dxselector"))
	end
	...
	if not (iIsNum and not iNInRange) then
		error(dgsGenAsrt(i, "dgsSelectorSetItemTextSize", 2, "number", "1~"..iLen, iNInRange and "Out Of Range"))
	end

	if not (type(sizeX) == "number") then
		error(dgsGenAsrt(sizeX, "dgsSelectorSetItemTextSize", 3, "number"))
	end
...
function dgsSelectorGetItemTextSize(selector, i)
	if dgsGetType(selector) ~= "dgs-dxselector" then
		error(dgsGenAsrt(selector, "dgsSelectorGetItemTextSize", 1, "dgs-dxselector"))
	end
	...
	if not (iIsNum and not iNInRange) then
		error(dgsGenAsrt(i, "dgsSelectorGetItemTextSize", 2, "number", "1~"..iLen, iNInRange and "Out Of Range"))
	end
...
function dgsSelectorSetItemAlignment(selector, i, alignX, alignY)
	if dgsGetType(selector) ~= "dgs-dxselector" then
		error(dgsGenAsrt(selector, "dgsSelectorSetItemAlignment", 1, "dgs-dxselector"))
	end
	...
	if not (iIsNum and not iNInRange) then
		error(dgsGenAsrt(i, "dgsSelectorSetItemAlignment", 2, "number", "1~"..iLen, iNInRange and "Out Of Range"))
	end

	if not (alignX == nil or HorizontalAlign[alignX]) then
		error(dgsGenAsrt(alignX, "dgsSelectorSetItemAlignment", 3, "string", "left/center/right"))
	end

	if not (alignY == nil or VerticalAlign[alignY]) then
		error(dgsGenAsrt(alignY, "dgsSelectorSetItemAlignment", 4, "string", "top/center/bottom"))
	end
...
function dgsSelectorGetItemAlignment(selector, i)
	if dgsGetType(selector) ~= "dgs-dxselector" then
		error(dgsGenAsrt(selector, "dgsSelectorGetItemAlignment", 1, "dgs-dxselector"))
	end
	...
	if not (iIsNum and not iNInRange) then
		error(dgsGenAsrt(i, "dgsSelectorGetItemAlignment", 2, "number", "1~"..iLen, iNInRange and "Out Of Range"))
	end
...
function dgsSelectorSetItemImage(selector, i, image, color, offx, offy, w, h, relative)
	if dgsGetType(selector) ~= "dgs-dxselector" then
		error(dgsGenAsrt(selector, "dgsSelectorSetItemImage", 1, "dgs-dxselector"))
	end
	...
	if not (iIsNum and not iNInRange) then
		error(dgsGenAsrt(i, "dgsSelectorSetItemImage", 2, "number", "1~"..iLen, iNInRange and "Out Of Range"))
	end
...
function dgsSelectorGetItemImage(selector, i)
	if dgsGetType(selector) ~= "dgs-dxselector" then
		error(dgsGenAsrt(selector, "dgsSelectorGetItemImage", 1, "dgs-dxselector"))
	end
	...
	if not (iIsNum and not iNInRange) then
		error(dgsGenAsrt(i, "dgsSelectorGetItemImage", 2, "number", "1~"..iLen, iNInRange and "Out Of Range"))
	end
...
function dgsSelectorRemoveItemImage(selector, i)
	if dgsGetType(selector) ~= "dgs-dxselector" then
		error(dgsGenAsrt(selector, "dgsSelectorRemoveItemImage", 1, "dgs-dxselector"))
	end
	...
	if not (iIsNum and not iNInRange) then
		error(dgsGenAsrt(i, "dgsSelectorRemoveItemImage", 2, "number", "1~"..iLen, iNInRange and "Out Of Range"))
	end
