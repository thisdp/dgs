...
function dgsCreateDetectArea(...)
	...
	if not x then
		...
		if not(type(x) == "number") then
			error(dgsGenAsrt(x, "dgsCreateDetectArea", 1, "number"))
		end

		if not(type(y) == "number") then
			error(dgsGenAsrt(y, "dgsCreateDetectArea", 2, "number"))
		end

		if not(type(w) == "number") then
			error(dgsGenAsrt(w, "dgsCreateDetectArea", 3, "number"))
		end

		if not(type(h) == "number") then
			error(dgsGenAsrt(h, "dgsCreateDetectArea", 4, "number"))
		end
...
function dgsDetectAreaSetFunction(detectarea, fncStr)
	if dgsGetType(detectarea) ~= "dgs-dxdetectarea" then
		error(dgsGenAsrt(detectarea, "dgsDetectAreaSetFunction", 1, "dgs-dxdetectarea"))
	end

	if not (dgsIsType(fncStr, "string") or dgsIsType(fncStr, "texture") or dgsIsType(fncStr, "svg")) then
		error(dgsGenAsrt(fncStr, "dgsDetectAreaSetFunction", 2, "string/texture"))
	end
	...
	if type(fncStr) == "string" then
		...
		if not fnc then
			error(dgsGenAsrt(fnc, "dgsDetectAreaSetFunction", 2, _, _, _, "Failed to load function:"..err))
		end
...
function dgsDetectAreaSetDebugModeEnabled(detectarea, state, alpha)
	if dgsGetType(detectarea) ~= "dgs-dxdetectarea" then
		error(dgsGenAsrt(detectarea, "dgsDetectAreaSetDebugModeEnabled", 1, "dgs-dxdetectarea"))
	end
...
function dgsDetectAreaGetDebugModeEnabled(detectarea)
	if dgsGetType(detectarea) ~= "dgs-dxdetectarea" then
		error(dgsGenAsrt(detectarea, "dgsDetectAreaGetDebugModeEnabled", 1, "dgs-dxdetectarea"))
	end
...
function dgsDetectAreaUpdateDebugView(detectarea)
	if not dgsElementData[detectarea].debugMode then
		return
	end
