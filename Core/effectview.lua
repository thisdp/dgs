...
function dgsCreateEffectView(...)
	...
	if not(type(x) == "number") then
		error(dgsGenAsrt(x, "dgsCreateEffectView", 1, "number"))
	end

	if not(type(y) == "number") then
		error(dgsGenAsrt(y, "dgsCreateEffectView", 2, "number"))
	end

	if not(type(w) == "number") then
		error(dgsGenAsrt(w, "dgsCreateEffectView", 3, "number"))
	end

	if not(type(h) == "number") then
		error(dgsGenAsrt(h, "dgsCreateEffectView", 4, "number"))
	end
...
function dgsEffectViewSetEffectShader(effectview, effect)
	if not dgsIsType(effectview, "dgs-dxeffectview") then
		error(dgsGenAsrt(effectview, "dgsEffectViewSetEffectShader", 1, "dgs-dxeffectview"))
	end

	local eleData = dgsElementData[effectview]

	if effect then
		if not isElement(effect) or getElementType(effect) ~= "shader" then
			error(dgsGenAsrt(effect, "dgsEffectViewSetEffectShader", 2, "shader"))
		end
