...
function dgsCreateEdit(...)
	...
	if not(type(x) == "number") then
		error(dgsGenAsrt(x, "dgsCreateEdit", 1, "number"))
	end

	if not(type(y) == "number") then
		error(dgsGenAsrt(y, "dgsCreateEdit", 2, "number"))
	end

	if not(type(w) == "number") then
		error(dgsGenAsrt(w, "dgsCreateEdit", 3, "number"))
	end

	if not(type(h) == "number") then
		error(dgsGenAsrt(h, "dgsCreateEdit", 4, "number"))
	end

	if relative then
		if x > 100 or x < -100 then
			error(dgsGenAsrt(x, "dgsCreateEdit", 1, "float between [0, 1]"))
		end

		if y > 100 or y < -100 then
			error(dgsGenAsrt(y, "dgsCreateEdit", 2, "float between [0, 1]"))
		end

		if w > 10 or w < -10 then
			error(dgsGenAsrt(w, "dgsCreateEdit", 3, "float between [0, 1]"))
		end

		if h > 10 or h < -10 then
			error(dgsGenAsrt(h, "dgsCreateEdit", 4, "float between [0, 1]"))
		end
	end
	...
	dgsAddEventHandler("onDgsTextChange", edit, "dgsEditOnTextChange", false)
...
function dgsEditRecreateRenderTarget(edit, lateAlloc)
	...
	if isElement(eleData.bgRT) then
		destroyElement(eleData.bgRT)
	end
...
function dgsEditBlur()
	...
	if not dgsElementData[GlobalEdit] then
		dgsElementData[GlobalEdit] = {}
	end
...
function dgsEditOnTextChange()
	local eleData = dgsElementData[source]

	--Check RTL
	if eleData.rtlAutoDetect then
		local text = eleData.text

		if utf8.isContainsArabic(text) then
			dgsSetData(source, "rtl", true)
		else
			dgsSetData(source, "rtl", false)
		end
	end
...
function dgsEditCheckPreSwitch(isGoPrevious)
	if not wasEventCancelled() then
		if not dgsElementData[source].enableTabSwitch then
			return
		end
		...
		if isGoPrevious then
			...
			for i = 1, #theTable do
				while(true) do
					...
					if dgsGetType(newEdit) ~= "dgs-dxedit" then
						break
					end	--First it is an edit

					local eleData = dgsElementData[newEdit]

					if not eleData.enabled then
						break
					end  --is enabled

					if not eleData.visible then
						break
					end  --is visible
					...
					if not eleData.enableTabSwitch then
						break
					end --is tab key switch enabled
...
function dgsEditSetMasked(edit, masked)
	if not dgsIsType(edit, "dgs-dxedit") then
		error(dgsGenAsrt(edit, "dgsEditSetMasked", 1, "dgs-dxedit"))
	end
...
function dgsEditGetMasked(edit)
	if not dgsIsType(edit, "dgs-dxedit") then
		error(dgsGenAsrt(edit, "dgsEditGetMasked", 1, "dgs-dxedit"))
	end
...
function dgsEditMoveCaret(edit, offset, selectText)
	if not dgsIsType(edit, "dgs-dxedit") then
		error(dgsGenAsrt(edit, "dgsEditMoveCaret", 1, "dgs-dxedit"))
	end

	if not(type(offset) == "number") then
		error(dgsGenAsrt(offset, "dgsEditMoveCaret", 2, "number"))
	end
...
function dgsEditSetCaretPosition(edit, pos, doSelect)
	if not dgsIsType(edit, "dgs-dxedit") then
		error(dgsGenAsrt(edit, "dgsEditSetCaretPosition", 1, "dgs-dxedit"))
	end

	if not(type(pos) == "number" or pos == nil) then
		error(dgsGenAsrt(pos, "dgsEditSetCaretPosition", 2, "nil/number"))
	end
...
function dgsEditGetCaretPosition(edit)
	if not dgsIsType(edit, "dgs-dxedit") then
		error(dgsGenAsrt(edit, "dgsEditGetCaretPosition", 1, "dgs-dxedit"))
	end
...
function dgsEditSetCaretStyle(edit, style)
	if not dgsIsType(edit, "dgs-dxedit") then
		error(dgsGenAsrt(edit, "dgsEditSetCaretStyle", 1, "dgs-dxedit"))
	end

	if not(type(style) == "number") then
		error(dgsGenAsrt(style, "dgsEditSetCaretStyle", 2, "number"))
	end
...
function dgsEditGetCaretStyle(edit, style)
	if not dgsIsType(edit, "dgs-dxedit") then
		error(dgsGenAsrt(edit, "dgsEditGetCaretStyle", 1, "dgs-dxedit"))
	end
...
function dgsEditSetMaxLength(edit, maxLength, nonTextCut)
	if not dgsIsType(edit, "dgs-dxedit") then
		error(dgsGenAsrt(edit, "dgsEditSetMaxLength", 1, "dgs-dxedit"))
	end

	if not(type(maxLength) == "number") then
		error(dgsGenAsrt(maxLength, "dgsEditSetMaxLength", 2, "number"))
	end
...
function dgsEditGetMaxLength(edit)
	if not dgsIsType(edit, "dgs-dxedit") then
		error(dgsGenAsrt(edit, "dgsEditGetMaxLength", 1, "dgs-dxedit"))
	end
...
function dgsEditSetReadOnly(edit, state)
	if not dgsIsType(edit, "dgs-dxedit") then
		error(dgsGenAsrt(edit, "dgsEditSetReadOnly", 1, "dgs-dxedit"))
	end
...
function dgsEditGetReadOnly(edit)
	if not dgsIsType(edit, "dgs-dxedit") then
		error(dgsGenAsrt(edit, "dgsEditGetReadOnly", 1, "dgs-dxedit"))
	end
...
function dgsEditSetTextFilter(edit, str)
	if not dgsIsType(edit, "dgs-dxedit") then
		error(dgsGenAsrt(edit, "dgsEditSetTextFilter", 1, "dgs-dxedit"))
	end
...
function dgsEditInsertText(edit, index, text)
	if not dgsIsType(edit, "dgs-dxedit") then
		error(dgsGenAsrt(edit, "dgsEditInsertText", 1, "dgs-dxedit"))
	end

	if not(type(index) == "number") then
		error(dgsGenAsrt(index, "dgsEditInsertText", 2, "number"))
	end
...
function dgsEditDeleteText(edit, fromIndex, toIndex, noAffectCaret, historyRecState)
	if not dgsIsType(edit, "dgs-dxedit") then
		error(dgsGenAsrt(edit, "dgsEditDeleteText", 1, "dgs-dxedit"))
	end

	if not(type(fromIndex) == "number") then
		error(dgsGenAsrt(fromIndex, "dgsEditDeleteText", 2, "number"))
	end

	if not(type(toIndex) == "number") then
		error(dgsGenAsrt(toIndex, "dgsEditDeleteText", 3, "number"))
	end
...
function dgsEditClearText(edit)
	if not dgsIsType(edit, "dgs-dxedit") then
		error(dgsGenAsrt(edit, "dgsEditClearText", 1, "dgs-dxedit"))
	end
...
function dgsEditGetPartOfText(edit, fromIndex, toIndex, delete)
	if not dgsIsType(edit, "dgs-dxedit") then
		error(dgsGenAsrt(edit, "dgsEditGetPartOfText", 1, "dgs-dxedit"))
	end
...
function dgsEditSetHorizontalAlign(edit, align)
	if not dgsIsType(edit, "dgs-dxedit") then
		error(dgsGenAsrt(edit, "dgsEditSetHorizontalAlign", 1, "dgs-dxedit"))
	end

	if not HorizontalAlign[align] then
		error(dgsGenAsrt(align, "dgsEditSetHorizontalAlign", 2, "string", "left/center/right"))
	end
...
function dgsEditSetVerticalAlign(edit, align)
	if not dgsIsType(edit, "dgs-dxedit") then
		error(dgsGenAsrt(edit, "dgsEditSetVerticalAlign", 1, "dgs-dxedit"))
	end

	if not VerticalAlign[align] then
		error(dgsGenAsrt(align, "dgsEditSetVerticalAlign", 2, "string", "top/center/bottom"))
	end
...
function dgsEditGetHorizontalAlign(edit)
	if not dgsIsType(edit, "dgs-dxedit") then
		error(dgsGenAsrt(edit, "dgsEditGetHorizontalAlign", 1, "dgs-dxedit"))
	end
...
function dgsEditGetVerticalAlign(edit)
	if not dgsIsType(edit, "dgs-dxedit") then
		error(dgsGenAsrt(edit, "dgsEditGetVerticalAlign", 1, "dgs-dxedit"))
	end
...
function dgsEditSetUnderlined(edit, state)
	if not dgsIsType(edit, "dgs-dxedit") then
		error(dgsGenAsrt(edit, "dgsEditSetUnderlined", 1, "dgs-dxedit"))
	end
...
function dgsEditGetUnderlined(edit, state)
	if not dgsIsType(edit, "dgs-dxedit") then
		error(dgsGenAsrt(edit, "dgsEditGetUnderlined", 1, "dgs-dxedit"))
	end
...
function searchEditMousePosition(edit, posx)
	...
	local hAlign = alignment[1]
	local rtl = eleData.rtl

	if rtl then	--RTL will swap left/right horizontal alignment
...
function dgsEditSetTypingSound(edit, path, volume)
	if not dgsIsType(edit, "dgs-dxedit") then
		error(dgsGenAsrt(edit, "dgsEditSetTypingSound", 1, "dgs-dxedit"))
	end

	if not(type(path) == "string") then
		error(dgsGenAsrt(path, "dgsEditSetTypingSound", 2, "string"))
	end
	...
	if not fileExists(path) then
		error(dgsGenAsrt(path, "dgsEditSetTypingSound", 2, _, _, _, "Couldn't find such file '"..path.."'"))
	end
...
function dgsEditGetTypingSound(edit)
	if not dgsIsType(edit, "dgs-dxedit") then
		error(dgsGenAsrt(edit, "dgsEditGetTypingSound", 1, "dgs-dxedit"))
	end
...
function dgsEditSetTypingSoundVolume(edit, volume)
	if dgsGetType(edit) ~= "dgs-dxedit" then
		error(dgsGenAsrt(edit, "dgsEditSetTypingSoundVolume", 1, "dgs-dxedit"))
	end

	if type(volume) ~= "number" then
		error(dgsGenAsrt(volume, "dgsEditSetTypingSoundVolume", 2, "number"))
	end
...
function dgsEditGetTypingSoundVolume(edit)
	if dgsGetType(edit) ~= "dgs-dxedit" then
		error(dgsGenAsrt(edit, "dgsEditGetTypingSoundVolume", 1, "dgs-dxedit"))
	end
...
function dgsEditSetAlignment(edit, horizontal, vertical)
	if not dgsIsType(edit, "dgs-dxedit") then
		error(dgsGenAsrt(edit, "dgsEditSetAlignment", 1, "dgs-dxedit"))
	end

	if not (not horizontal or HorizontalAlign[horizontal]) then
		error(dgsGenAsrt(horizontal, "dgsEditSetAlignment", 2, "string", "left/center/right"))
	end

	if not (not vertical or VerticalAlign[vertical]) then
		error(dgsGenAsrt(vertical, "dgsEditSetAlignment", 3, "string", "top/center/bottom"))
	end
...
function dgsEditGetAlignment(edit)
	if not dgsIsType(edit, "dgs-dxedit") then
		error(dgsGenAsrt(edit, "dgsEditGetAlignment", 1, "dgs-dxedit"))
	end
...
function dgsEditSetPlaceHolder(edit, placeHolder)
	if not dgsIsType(edit, "dgs-dxedit") then
		error(dgsGenAsrt(edit, "dgsEditSetPlaceHolder", 1, "dgs-dxedit"))
	end
...
function dgsEditGetPlaceHolder(edit)
	if not dgsIsType(edit, "dgs-dxedit") then
		error(dgsGenAsrt(edit, "dgsEditGetPlaceHolder", 1, "dgs-dxedit"))
	end
...
function dgsEditAutoCompleteAddParameterFunction(parameterType, parameterFunction)
	if not dgsIsType(parameterType, "string") then
		error(dgsGenAsrt(parameterType, "dgsEditAddAutoCompleteParameterFunction", 1, "string"))
	end

	if not dgsIsType(parameterFunction, "string") then
		error(dgsGenAsrt(parameterFunction, "dgsEditAddAutoCompleteParameterFunction", 2, "string"))
	end

	local fnc, err = loadstring(parameterFunction)

	if err then
		error(dgsGenAsrt(parameterFunction, "dgsEditAddAutoCompleteParameterFunction", _, _, _, err))
	end
...
function dgsEditAutoCompleteRemoveParameterFunction(parameterType)
	if not dgsIsType(parameterType, "string") then
		error(dgsGenAsrt(parameterType, "dgsEditRemoteAutoCompleteParameterFunction", 1, "string"))
	end
...
function dgsEditAddAutoComplete(edit, str, isSensitive, isAdvanced)
	if not dgsIsType(edit, "dgs-dxedit") then
		error(dgsGenAsrt(edit, "dgsEditAddAutoComplete", 1, "dgs-dxedit"))
	end
	...
	if strTyp == "table" then
		if isAdvanced then	--Add Advanced autoComplete
			...
			if not autoComplete[identifier] then
				dgsElementData[edit].autoCompleteCount = dgsElementData[edit].autoCompleteCount+1
			end
...
function dgsEditSetAutoComplete(edit, acTable)
	if not dgsIsType(edit, "dgs-dxedit") then
		error(dgsGenAsrt(edit, "dgsEditSetAutoComplete", 1, "dgs-dxedit"))
	end

	if not (type(acTable) == "table") then
		error(dgsGenAsrt(acTable, "dgsEditSetAutoComplete", 2, "table"))
	end
...
function dgsEditDeleteAutoComplete(edit, str)
	if not dgsIsType(edit, "dgs-dxedit") then
		error(dgsGenAsrt(edit, "dgsEditDeleteAutoComplete", 1, "dgs-dxedit"))
	end
...
function dgsEditGetAutoComplete(edit)
	if not dgsIsType(edit, "dgs-dxedit") then
		error(dgsGenAsrt(edit, "dgsEditGetAutoComplete", 1, "dgs-dxedit"))
	end
...
dgsRenderer["dgs-dxedit"] = function(source, x, y, w, h, mx, my, cx, cy, enabledInherited, enabledSelf, eleData, parentAlpha, isPostGUI, rndtgt)
	...
	if eleData.bgRT and (eleData.updateRTNextFrame or dgsRenderInfo.RTRestoreNeed) then
		...
		if not placeHolderIgnoreRndTgt then
			if isPlaceHolderShown then
				...
				dgsDrawText(placeHolder, textLeft+placeHolderOffset[1], textTop+placeHolderOffset[2], textRight-posFix+placeHolderOffset[1], textBottom+placeHolderOffset[2], pColor, pHolderTextSizeX, pHolderTextSizeY, pFont, hAlign, alignment[2], false, false, false, pColorcoded, subPixelPos, 0, 0, 0, 0, shadowOffsetX, shadowOffsetY, shadowColor, shadowIsOutline, shadowFont)
			end
		end

		if eleData.autoCompleteShow then
			dgsDrawText(eleData.autoCompleteShow.result or "", textLeft, textTop, textRight-posFix, textBottom, eleData.autoCompleteTextColor or applyColorAlpha(textColor, 0.7*parentAlpha), txtSizX, txtSizY, font, hAlign, alignment[2], false, false, false, false, subPixelPos, 0, 0, 0, 0, shadowOffsetX, shadowOffsetY, shadowColor, shadowIsOutline, shadowFont)
		end

		dgsDrawText(text, textLeft, textTop, textRight-posFix, textBottom, applyColorAlpha(textColor, parentAlpha), txtSizX, txtSizY, font, hAlign, alignment[2], false, false, false, false, subPixelPos, 0, 0, 0, 0, shadowOffsetX, shadowOffsetY, shadowColor, shadowIsOutline, shadowFont)
	end
	...
	if placeHolderIgnoreRndTgt then
		if isPlaceHolderShown then
			...
			dxSetBlendMode(rndtgt and "modulate_add" or "blend")
			dgsDrawText(placeHolder, px+textLeft+placeHolderOffset[1], py+placeHolderOffset[2], px+textRight-posFix+placeHolderOffset[1], py+textBottom+placeHolderOffset[2], pColor, txtSizX, txtSizY, pFont, hAlign, alignment[2], false, false, isPostGUI, pColorcoded, subPixelPos, 0, 0, 0, 0, shadowOffsetX, shadowOffsetY, shadowColor, shadowIsOutline, shadowFont)
		end
	end
