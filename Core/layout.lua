...
function dgsCreateLayout(...)
	...
	if not(type(x) == "number") then
		error(dgsGenAsrt(x, "dgsCreateLayout", 1, "number"))
	end

	if not(type(y) == "number") then
		error(dgsGenAsrt(y, "dgsCreateLayout", 2, "number"))
	end

	if not(type(w) == "number") then
		error(dgsGenAsrt(w, "dgsCreateLayout", 3, "number"))
	end

	if not(type(h) == "number") then
		error(dgsGenAsrt(h, "dgsCreateLayout", 4, "number"))
	end

	if builtinLayoutStyle[layoutStyle] then
		layoutStyle = builtinLayoutStyle[layoutStyle]
	else
		if dgsGetType(layoutStyle) ~= "string" then
			error(dgsGenAsrt(layoutStyle, "dgsCreateLayout", 5, "string"))
		end

		local fnc, err = loadstring("return "..layoutStyle)

		if not fnc then
			error(dgsGenAsrt(fnc, "dgsCreateLayout", 5, _, _, _, "Failed to load layout style function:"..err))
		end

		local tempLayoutStyle = fnc()

		if type(tempLayoutStyle) ~= "table" then
			error(dgsGenAsrt(tempLayoutStyle, "dgsCreateLayout", 5, "table", _, _, "Failed to load layout style"))
		end
	...
	dgsApplyGeneralProperties(layout, sRes)
...
function dgsLayoutAddItem(layout, item, sortPriority)
	if dgsGetType(layout) ~= "dgs-dxlayout" then
		error(dgsGenAsrt(layout, "dgsCreateLayout", 1, "dgs-dxlayout"))
	end

	if not dgsIsType(item) then
		error(dgsGenAsrt(item, "dgsCreateLayout", 2, "dgs-dxelement"))
	end

	if not (not sortPriority or type(sortPriority) == "number") then
		error(dgsGenAsrt(item, "dgsLayoutAddItem", 1, "number/nil"))
	end
	...
	if onItemAdd then
		if not sortPriority then
			...
			if dgsGetParent(item) ~= layout then
				dgsSetParent(item, layout)
			end
...
function dgsLayoutSetItemSortPriority(item, sortPriority)
	if not dgsIsType(item) then
		error(dgsGenAsrt(item, "dgsLayoutSetItemSortPriority", 1, "dgs-dxelement"))
	end

	if not (not sortPriority or type(sortPriority) == "number") then
		error(dgsGenAsrt(item, "dgsLayoutSetItemSortPriority", 1, "number/nil"))
	end

	local layout = dgsGetParent(item)

	if dgsGetType(layout) ~= "dgs-dxlayout" then
		error(dgsGenAsrt(layout, "dgsLayoutSetItemSortPriority", 1, "dgs-dxlayout as parent"))
	end
...
function dgsLayoutGetItemSortPriority(item)
	if not dgsIsType(item) then
		error(dgsGenAsrt(item, "dgsLayoutGetItemSortPriority", 1, "dgs-dxelement"))
	end

	local layout = dgsGetParent(item)

	if dgsGetType(layout) ~= "dgs-dxlayout" then
		error(dgsGenAsrt(layout, "dgsLayoutGetItemSortPriority", 1, "dgs-dxlayout as parent"))
	end
...
function dgsLayoutRemoveItem(layout, item)
	if dgsGetType(layout) ~= "dgs-dxlayout" then
		error(dgsGenAsrt(layout, "dgsLayoutRemoveItem", 1, "dgs-dxlayout"))
	end

	if not dgsIsType(item) then
		error(dgsGenAsrt(item, "dgsLayoutRemoveItem", 2, "dgs-dxelement"))
	end
...
function dgsLayoutGetItemIndex(layout, item)
	if dgsGetType(layout) ~= "dgs-dxlayout" then
		error(dgsGenAsrt(layout, "dgsLayoutGetItemIndex", 1, "dgs-dxlayout"))
	end

	if not dgsIsType(item) then
		error(dgsGenAsrt(item, "dgsLayoutGetItemIndex", 2, "dgs-dxelement"))
	end
