...
function dgsSetCustomCursorImage(cursorType, image, rotation, rotationCenter, offset, scale)
	...
	elseif sStyle[cursorType] then
		if not(type(cursorType) == "string" and cursorTypesBuiltIn[cursorType]) then
			error(dgsGenAsrt(cursorType, "dgsSetCustomCursorImage", 1, "string", _, _, "unsupported type"))
		end

		if image then
			if not(isMaterial(image)) then
				error(dgsGenAsrt(image, "dgsSetCustomCursorImage", 2, "material"))
			end
...
function dgsGetCustomCursorImage(cursorType)
	if not(type(cursorType) == "string" and cursorTypesBuiltIn[cursorType]) then
		error(dgsGenAsrt(cursorType, "dgsGetCustomCursorImage", 1, "string", _, _, "unsupported type"))
	end
...
function dgsSetCustomCursorSize(size)
	if not(type(size) == "number") then
		error(dgsGenAsrt(size, "dgsSetCustomCursorSize", 1, "number"))
	end
...
function dgsSetCustomCursorColor(color)
	if not(type(color) == "number") then
		error(dgsGenAsrt(color, "dgsSetCustomCursorColor", 1, "number"))
	end
