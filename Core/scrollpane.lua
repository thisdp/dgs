...
function dgsCreateScrollPane(...)
	...
	if not(type(x) == "number") then
		error(dgsGenAsrt(x, "dgsCreateScrollPane", 1, "number"))
	end

	if not(type(y) == "number") then
		error(dgsGenAsrt(y, "dgsCreateScrollPane", 2, "number"))
	end

	if not(type(w) == "number") then
		error(dgsGenAsrt(w, "dgsCreateScrollPane", 3, "number"))
	end

	if not(type(h) == "number") then
		error(dgsGenAsrt(h, "dgsCreateScrollPane", 4, "number"))
	end

	if relative then
		if x > 100 or x < -100 then
			error(dgsGenAsrt(x, "dgsCreateScrollPane", 1, "float between [0, 1]"))
		end

		if y > 100 or y < -100 then
			error(dgsGenAsrt(y, "dgsCreateScrollPane", 2, "float between [0, 1]"))
		end

		if w > 10 or w < -10 then
			error(dgsGenAsrt(w, "dgsCreateScrollPane", 3, "float between [0, 1]"))
		end

		if h > 10 or h < -10 then
			error(dgsGenAsrt(h, "dgsCreateScrollPane", 4, "float between [0, 1]"))
		end
	end
...
function dgsScrollPaneRecreateRenderTarget(scrollpane, lateAlloc)
	...
	if isElement(eleData.mainRT) then
		destroyElement(eleData.mainRT)
	end
...
function dgsScrollPaneGetScrollBar(scrollpane)
	if not dgsIsType(scrollpane, "dgs-dxscrollpane") then
		error(dgsGenAsrt(scrollpane, "dgsScrollPaneGetScrollBar", 1, "dgs-dxscrollpane"))
	end
...
function dgsScrollPaneSetScrollPosition(scrollpane, vertical, horizontal)
	if not dgsIsType(scrollpane, "dgs-dxscrollpane") then
		error(dgsGenAsrt(scrollpane, "dgsScrollPaneSetScrollPosition", 1, "dgs-dxscrollpane"))
	end

	if vertical and not (type(vertical) == "number" and vertical>= 0 and vertical <= 100) then
		error(dgsGenAsrt(vertical, "dgsScrollPaneSetScrollPosition", 2, "nil/number", "0~100"))
	end

	if horizontal and not (type(horizontal) == "number" and horizontal>= 0 and horizontal <= 100) then
		error(dgsGenAsrt(horizontal, "dgsScrollPaneSetScrollPosition", 3, "nil/number", "0~100"))
	end
...
function dgsScrollPaneGetViewOffset(scrollpane)
	if not dgsIsType(scrollpane, "dgs-dxscrollpane") then
		error(dgsGenAsrt(scrollpane, "dgsScrollPaneGetViewOffset", 1, "dgs-dxscrollpane"))
	end
...
function dgsScrollPaneSetViewOffset(scrollpane, x, y)
	if not dgsIsType(scrollpane, "dgs-dxscrollpane") then
		error(dgsGenAsrt(scrollpane, "dgsScrollPaneSetViewOffset", 1, "dgs-dxscrollpane"))
	end
...
function dgsScrollPaneGetHorizontalScrollPosition(scrollpane)
	if not dgsIsType(scrollpane, "dgs-dxscrollpane") then
		error(dgsGenAsrt(scrollpane, "dgsScrollPaneGetHorizontalScrollPosition", 1, "dgs-dxscrollpane"))
	end
...
function dgsScrollPaneSetHorizontalScrollPosition(scrollpane, horizontal)
	if not dgsIsType(scrollpane, "dgs-dxscrollpane") then
		error(dgsGenAsrt(scrollpane, "dgsScrollPaneSetHorizontalScrollPosition", 1, "dgs-dxscrollpane"))
	end

	if horizontal and not (type(horizontal) == "number" and horizontal>= 0 and horizontal <= 100) then
		error(dgsGenAsrt(horizontal, "dgsScrollPaneSetHorizontalScrollPosition", 3, "nil/number", "0~100"))
	end
...
function dgsScrollPaneGetVerticalScrollPosition(scrollpane)
	if not dgsIsType(scrollpane, "dgs-dxscrollpane") then
		error(dgsGenAsrt(scrollpane, "dgsScrollPaneGetVerticalScrollPosition", 1, "dgs-dxscrollpane"))
	end
...
function dgsScrollPaneSetVerticalScrollPosition(scrollpane, vertical)
	if not dgsIsType(scrollpane, "dgs-dxscrollpane") then
		error(dgsGenAsrt(scrollpane, "dgsScrollPaneSetVerticalScrollPosition", 1, "dgs-dxscrollpane"))
	end

	if vertical and not (type(vertical) == "number" and vertical>= 0 and vertical <= 100) then
		error(dgsGenAsrt(vertical, "dgsScrollPaneSetVerticalScrollPosition", 2, "nil/number", "0~100"))
	end
...
function dgsScrollPaneGetScrollPosition(scrollpane)
	if not dgsIsType(scrollpane, "dgs-dxscrollpane") then
		error(dgsGenAsrt(scrollpane, "dgsScrollPaneGetScrollPosition", 1, "dgs-dxscrollpane"))
	end
...
function dgsScrollPaneSetScrollBarState(scrollpane, vertical, horizontal)
	if not dgsIsType(scrollpane, "dgs-dxscrollpane") then
		error(dgsGenAsrt(scrollpane, "dgsScrollPaneSetScrollBarState", 1, "dgs-dxscrollpane"))
	end
