...
function dgsCreate3DImage(...)
	...
	if not(type(x) == "number") then
		error(dgsGenAsrt(x, "dgsCreate3DImage", 1, "number"))
	end

	if not(type(y) == "number") then
		error(dgsGenAsrt(y, "dgsCreate3DImage", 2, "number"))
	end

	if not(type(z) == "number") then
		error(dgsGenAsrt(z, "dgsCreate3DImage", 3, "number"))
	end
...
function dgs3DImageGetImage(image)
	if not(dgsGetType(image) == "dgs-dx3dimage") then
		error(dgsGenAsrt(image, "dgs3DImageGetImage", 1, "dgs-dx3dimage"))
	end
...
function dgs3DImageSetImage(image, imgTex)
	if not(dgsGetType(image) == "dgs-dx3dimage") then
		error(dgsGenAsrt(image, "dgs3DImageSetImage", 1, "dgs-dx3dimage"))
	end
...
function dgs3DImageSetSize(image, w, h)
	if not(dgsGetType(image) == "dgs-dx3dimage") then
		error(dgsGenAsrt(image, "dgs3DImageSetSize", 1, "dgs-dx3dimage"))
	end

	if not(type(w) == "number") then
		error(dgsGenAsrt(w, "dgs3DImageSetSize", 2, "number"))
	end

	if not(type(h) == "number") then
		error(dgsGenAsrt(h, "dgs3DImageSetSize", 3, "number"))
	end
...
function dgs3DImageGetSize(image)
	if not(dgsGetType(image) == "dgs-dx3dimage") then
		error(dgsGenAsrt(image, "dgs3DImageGetSize", 1, "dgs-dx3dimage"))
	end
...
function dgs3DImageAttachToElement(image, element, offX, offY, offZ)
	if not(dgsGetType(image) == "dgs-dx3dimage") then
		error(dgsGenAsrt(image, "dgs3DImageAttachToElement", 1, "dgs-dx3dimage"))
	end

	if not(isElement(element)) then
		error(dgsGenAsrt(element, "dgs3DImageAttachToElement", 2, "element"))
	end
...
function dgs3DImageIsAttached(image)
	if not(dgsGetType(image) == "dgs-dx3dimage") then
		error(dgsGenAsrt(image, "dgs3DImageIsAttached", 1, "dgs-dx3dimage"))
	end
...
function dgs3DImageDetachFromElement(image)
	if not(dgsGetType(image) == "dgs-dx3dimage") then
		error(dgsGenAsrt(image, "dgs3DImageDetachFromElement", 1, "dgs-dx3dimage"))
	end
...
function dgs3DImageSetAttachedOffsets(image, offX, offY, offZ)
	if not(dgsGetType(image) == "dgs-dx3dimage") then
		error(dgsGenAsrt(image, "dgs3DImageSetAttachedOffsets", 1, "dgs-dx3dimage"))
	end
...
function dgs3DImageGetAttachedOffsets(image)
	if not(dgsGetType(image) == "dgs-dx3dimage") then
		error(dgsGenAsrt(image, "dgs3DImageGetAttachedOffsets", 1, "dgs-dx3dimage"))
	end
...
function dgs3DImageSetUVSize(image, sx, sy, relative)
	if dgsGetType(image) ~= "dgs-dx3dimage" then
		error(dgsGenAsrt(image, "dgs3DImageSetUVSize", 1, "dgs-dx3dimage"))
	end
...
function dgs3DImageGetUVSize(image, relative)
	if dgsGetType(image) ~= "dgs-dx3dimage" then
		error(dgsGenAsrt(image, "dgs3DImageGetUVSize", 1, "dgs-dx3dimage"))
	end
...
function dgs3DImageSetUVPosition(image, x, y, relative)
	if dgsGetType(image) ~= "dgs-dx3dimage" then
		error(dgsGenAsrt(image, "dgs3DImageSetUVPosition", 1, "dgs-dx3dimage"))
	end
...
function dgs3DImageGetUVPosition(image, relative)
	if dgsGetType(image) ~= "dgs-dx3dimage" then
		error(dgsGenAsrt(image, "dgs3DImageGetUVPosition", 1, "dgs-dx3dimage"))
	end
...
function dgs3DImageGetNativeSize(image)
	if dgsGetType(image) ~= "dgs-dx3dimage" then
		error(dgsGenAsrt(image, "dgs3DImageGetNativeSize", 1, "dgs-dx3dimage"))
	end
	...
	if isRender then
		...
		if distance <= maxDistance and distance > 0 then
			...
			if canBeBlocked then
				...
				if canBeBlocked.checkBuildings == nil then
					canBeBlocked.checkBuildings = g_canBeBlocked.checkBuildings
				end

				if canBeBlocked.checkVehicles == nil then
					canBeBlocked.checkVehicles = g_canBeBlocked.checkVehicles
				end

				if canBeBlocked.checkPeds == nil then
					canBeBlocked.checkPeds = g_canBeBlocked.checkPeds
				end

				if canBeBlocked.checkObjects == nil then
					canBeBlocked.checkObjects = g_canBeBlocked.checkObjects
				end

				if canBeBlocked.checkDummies == nil then
					canBeBlocked.checkDummies = g_canBeBlocked.checkDummies
				end

				if canBeBlocked.seeThroughStuff == nil then
					canBeBlocked.seeThroughStuff = g_canBeBlocked.seeThroughStuff
				end

				if canBeBlocked.ignoreSomeObjectsForCamera == nil then
					canBeBlocked.ignoreSomeObjectsForCamera = g_canBeBlocked.ignoreSomeObjectsForCamera
				end
			end
