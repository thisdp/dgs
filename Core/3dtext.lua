...
function dgsCreate3DText(...)
	...
	if not(type(x) == "number") then
		error(dgsGenAsrt(x, "dgsCreate3DText", 1, "number"))
	end

	if not(type(y) == "number") then
		error(dgsGenAsrt(y, "dgsCreate3DText", 2, "number"))
	end

	if not(type(z) == "number") then
		error(dgsGenAsrt(z, "dgsCreate3DText", 3, "number"))
	end
	...
	dgsApplyGeneralProperties(text3d, sRes)
...
function dgs3DTextAttachToElement(text, element, offX, offY, offZ)
	if not(dgsGetType(text) == "dgs-dx3dtext") then
		error(dgsGenAsrt(text, "dgs3DTextAttachToElement", 1, "dgs-dx3dtext"))
	end

	if not isElement(element) then
		error(dgsGenAsrt(element, "dgs3DTextAttachToElement", 2, "element"))
	end
...
function dgs3DTextIsAttached(text)
	if not(dgsGetType(text) == "dgs-dx3dtext") then
		error(dgsGenAsrt(text, "dgs3DTextIsAttached", 1, "dgs-dx3dtext"))
	end
...
function dgs3DTextDetachFromElement(text)
	if not(dgsGetType(text) == "dgs-dx3dtext") then
		error(dgsGenAsrt(text, "dgs3DTextDetachFromElement", 1, "dgs-dx3dtext"))
	end
...
function dgs3DTextSetAttachedOffsets(text, offX, offY, offZ)
	if not(dgsGetType(text) == "dgs-dx3dtext") then
		error(dgsGenAsrt(text, "dgs3DTextSetAttachedOffsets", 1, "dgs-dx3dtext"))
	end
...
function dgs3DTextGetAttachedOffsets(text)
	if not(dgsGetType(text) == "dgs-dx3dtext") then
		error(dgsGenAsrt(text, "dgs3DTextGetAttachedOffsets", 1, "dgs-dx3dtext"))
	end
