...
function dgsCreateComboBox(...)
	...
	if not(type(x) == "number") then
		error(dgsGenAsrt(x, "dgsCreateComboBox", 1, "number"))
	end

	if not(type(y) == "number") then
		error(dgsGenAsrt(y, "dgsCreateComboBox", 2, "number"))
	end

	if not(type(w) == "number") then
		error(dgsGenAsrt(w, "dgsCreateComboBox", 3, "number"))
	end

	if not(type(h) == "number") then
		error(dgsGenAsrt(h, "dgsCreateComboBox", 4, "number"))
	end

	if relative then
		if x > 100 or x < -100 then
			error(dgsGenAsrt(x, "dgsCreateComboBox", 1, "float between [0, 1]"))
		end

		if y > 100 or y < -100 then
			error(dgsGenAsrt(y, "dgsCreateComboBox", 2, "float between [0, 1]"))
		end

		if w > 10 or w < -10 then
			error(dgsGenAsrt(w, "dgsCreateComboBox", 3, "float between [0, 1]"))
		end

		if h > 10 or h < -10 then
			error(dgsGenAsrt(h, "dgsCreateComboBox", 4, "float between [0, 1]"))
		end
	end
...
function dgsComboBoxRecreateRenderTarget(combobox, lateAlloc)
	if isElement(dgsElementData[combobox].bgRT) then
		destroyElement(dgsElementData[combobox].bgRT)
	end
...
function dgsComboBoxCheckState(state)
	if not wasEventCancelled() then
		...
		if not isElement(box) then
			return false
		end	--Make sure box is not destroyed
...
function dgsComboBoxSetCaptionText(combobox, caption)
	if not dgsIsType(combobox, "dgs-dxcombobox") then
		error(dgsGenAsrt(combobox, "dgsComboBoxSetCaptionText", 1, "dgs-dxcombobox"))
	end
...
function dgsComboBoxGetText(combobox)
	if not dgsIsType(combobox, "dgs-dxcombobox") then
		error(dgsGenAsrt(combobox, "dgsComboBoxGetText", 1, "dgs-dxcombobox"))
	end
...
function dgsComboBoxGetCaptionText(combobox)
	if not dgsIsType(combobox, "dgs-dxcombobox") then
		error(dgsGenAsrt(combobox, "dgsComboBoxGetCaptionText", 1, "dgs-dxcombobox"))
	end
...
function dgsComboBoxSetEditEnabled(combobox, enabled)
	if not dgsIsType(combobox, "dgs-dxcombobox") then
		error(dgsGenAsrt(combobox, "dgsComboBoxSetEditEnabled", 1, "dgs-dxcombobox"))
	end
...
function dgsComboBoxGetEditEnabled(combobox)
	if not dgsIsType(combobox, "dgs-dxcombobox") then
		error(dgsGenAsrt(combobox, "dgsComboBoxGetEditEnabled", 1, "dgs-dxcombobox"))
	end
...
function dgsComboBoxSetBoxHeight(combobox, height, relative)
	if not dgsIsType(combobox, "dgs-dxcombobox") then
		error(dgsGenAsrt(combobox, "dgsComboBoxSetBoxHeight", 1, "dgs-dxcombobox"))
	end

	if not type(height) == "number" then
		error(dgsGenAsrt(height, "dgsComboBoxSetBoxHeight", 2, "number"))
	end
...
function dgsComboBoxGetBoxHeight(combobox, relative)
	if not dgsIsType(combobox, "dgs-dxcombobox") then
		error(dgsGenAsrt(combobox, "dgsComboBoxGetBoxHeight", 1, "dgs-dxcombobox"))
	end
...
function dgsComboBoxAddItem(combobox, text)
	if not dgsIsType(combobox, "dgs-dxcombobox") then
		error(dgsGenAsrt(combobox, "dgsComboBoxAddItem", 1, "dgs-dxcombobox"))
	end
...
function dgsComboBoxSetItemText(combobox, i, text)
	if not dgsIsType(combobox, "dgs-dxcombobox") then
		error(dgsGenAsrt(combobox, "dgsComboBoxSetItemText", 1, "dgs-dxcombobox"))
	end
	...
	if not (iIsNum and not iNInRange) then
		error(dgsGenAsrt(i, "dgsComboBoxSetItemText", 2, "number", "1~"..iLen, iNInRange and "Out Of Range"))
	end
...
function dgsComboBoxGetItemText(combobox, i)
	if not dgsIsType(combobox, "dgs-dxcombobox") then
		error(dgsGenAsrt(combobox, "dgsComboBoxGetItemText", 1, "dgs-dxcombobox"))
	end
	...
	if not (iIsNum and not iNInRange) then
		error(dgsGenAsrt(i, "dgsComboBoxGetItemText", 2, "number", "1~"..iLen, iNInRange and "Out Of Range"))
	end
...
function dgsComboBoxSetItemData(combobox, i, data, ...)
	if not dgsIsType(combobox, "dgs-dxcombobox") then
		error(dgsGenAsrt(combobox, "dgsComboBoxSetItemData", 1, "dgs-dxcombobox"))
	end
	...
	if not (iIsNum and not iNInRange) then
		error(dgsGenAsrt(i, "dgsComboBoxSetItemData", 2, "number", "1~"..iLen, iNInRange and "Out Of Range"))
	end
...
function dgsComboBoxGetItemData(combobox, i, key)
	if not dgsIsType(combobox, "dgs-dxcombobox") then
		error(dgsGenAsrt(combobox, "dgsComboBoxGetItemData", 1, "dgs-dxcombobox"))
	end
	...
	if not (iIsNum and not iNInRange) then
		error(dgsGenAsrt(i, "dgsComboBoxGetItemData", 2, "number", "1~"..iLen, iNInRange and "Out Of Range"))
	end
...
function dgsComboBoxGetItemCount(combobox)
	if not dgsIsType(combobox, "dgs-dxcombobox") then
		error(dgsGenAsrt(combobox, "dgsComboBoxGetItemCount", 1, "dgs-dxcombobox"))
	end
...
function dgsComboBoxSetItemColor(combobox, i, color)
	if not dgsIsType(combobox, "dgs-dxcombobox") then
		error(dgsGenAsrt(combobox, "dgsComboBoxSetItemColor", 1, "dgs-dxcombobox"))
	end
	...
	if not (iIsNum and not iNInRange) then
		error(dgsGenAsrt(i, "dgsComboBoxSetItemColor", 2, "number", "1~"..iLen, iNInRange and "Out Of Range"))
	end
...
function dgsComboBoxGetItemColor(combobox, i)
	if not dgsIsType(combobox, "dgs-dxcombobox") then
		error(dgsGenAsrt(combobox, "dgsComboBoxGetItemColor", 1, "dgs-dxcombobox"))
	end
	...
	if not (iIsNum and not iNInRange) then
		error(dgsGenAsrt(i, "dgsComboBoxGetItemColor", 2, "number", "1~"..iLen, iNInRange and "Out Of Range"))
	end
...
function dgsComboBoxSetItemFont(combobox, i, font)
	if not dgsIsType(combobox, "dgs-dxcombobox") then
		error(dgsGenAsrt(combobox, "dgsComboBoxSetItemFont", 1, "dgs-dxcombobox"))
	end
	...
	if not (iIsNum and not iNInRange) then
		error(dgsGenAsrt(i, "dgsComboBoxSetItemFont", 2, "number", "1~"..iLen, iNInRange and "Out Of Range"))
	end
...
function dgsComboBoxGetItemFont(combobox, i)
	if not dgsIsType(combobox, "dgs-dxcombobox") then
		error(dgsGenAsrt(combobox, "dgsComboBoxGetItemFont", 1, "dgs-dxcombobox"))
	end
	...
	if not (iIsNum and not iNInRange) then
		error(dgsGenAsrt(i, "dgsComboBoxGetItemFont", 2, "number", "1~"..iLen, iNInRange and "Out Of Range"))
	end
...
function dgsComboBoxSetItemImage(combobox, i, image, color, offx, offy, w, h, relative)
	if not dgsIsType(combobox, "dgs-dxcombobox") then
		error(dgsGenAsrt(combobox, "dgsComboBoxSetItemImage", 1, "dgs-dxcombobox"))
	end
	...
	if not (iIsNum and not iNInRange) then
		error(dgsGenAsrt(i, "dgsComboBoxSetItemImage", 2, "number", "1~"..iLen, iNInRange and "Out Of Range"))
	end
...
function dgsComboBoxGetItemImage(combobox, i)
	if not dgsIsType(combobox, "dgs-dxcombobox") then
		error(dgsGenAsrt(combobox, "dgsComboBoxGetItemImage", 1, "dgs-dxcombobox"))
	end
	...
	if not (iIsNum and not iNInRange) then
		error(dgsGenAsrt(i, "dgsComboBoxGetItemImage", 2, "number", "1~"..iLen, iNInRange and "Out Of Range"))
	end
...
function dgsComboBoxRemoveItemImage(combobox, i)
	if not dgsIsType(combobox, "dgs-dxcombobox") then
		error(dgsGenAsrt(combobox, "dgsComboBoxRemoveItemImage", 1, "dgs-dxcombobox"))
	end
	...
	if not (iIsNum and not iNInRange) then
		error(dgsGenAsrt(i, "dgsComboBoxRemoveItemImage", 2, "number", "1~"..iLen, iNInRange and "Out Of Range"))
	end
...
function dgsComboBoxSetItemBackGroundImage(combobox, i, imageDefault, imageHoving, imageSelected)
	if not dgsIsType(combobox, "dgs-dxcombobox") then
		error(dgsGenAsrt(combobox, "dgsComboBoxSetItemBackGroundImage", 1, "dgs-dxcombobox"))
	end
	...
	if not (iIsNum and not iNInRange) then
		error(dgsGenAsrt(i, "dgsComboBoxSetItemBackGroundImage", 2, "number", "1~"..iLen, iNInRange and "Out Of Range"))
	end
...
function dgsComboBoxGetItemBackGroundImage(combobox, i)
	if not dgsIsType(combobox, "dgs-dxcombobox") then
		error(dgsGenAsrt(combobox, "dgsComboBoxGetItemBackGroundImage", 1, "dgs-dxcombobox"))
	end
	...
	if not (iIsNum and not iNInRange) then
		error(dgsGenAsrt(i, "dgsComboBoxGetItemBackGroundImage", 2, "number", "1~"..iLen, iNInRange and "Out Of Range"))
	end
...
function dgsComboBoxSetItemBackGroundColor(combobox, i, colorDefault, colorHoving, colorSelected)
	if not dgsIsType(combobox, "dgs-dxcombobox") then
		error(dgsGenAsrt(combobox, "dgsComboBoxSetItemBackGroundColor", 1, "dgs-dxcombobox"))
	end
	...
	if not (iIsNum and not iNInRange) then
		error(dgsGenAsrt(i, "dgsComboBoxSetItemBackGroundColor", 2, "number", "1~"..iLen, iNInRange and "Out Of Range"))
	end
...
function dgsComboBoxGetItemBackGroundColor(combobox, i)
	if not dgsIsType(combobox, "dgs-dxcombobox") then
		error(dgsGenAsrt(combobox, "dgsComboBoxGetItemBackGroundColor", 1, "dgs-dxcombobox"))
	end
	...
	if not (iIsNum and not iNInRange) then
		error(dgsGenAsrt(i, "dgsComboBoxGetItemBackGroundColor", 2, "number", "1~"..iLen, iNInRange and "Out Of Range"))
	end
...
function dgsComboBoxSetState(combobox, state)
	if not dgsIsType(combobox, "dgs-dxcombobox") then
		error(dgsGenAsrt(combobox, "dgsComboBoxSetState", 1, "dgs-dxcombobox"))
	end
...
function dgsComboBoxGetState(combobox)
	if not dgsIsType(combobox, "dgs-dxcombobox") then
		error(dgsGenAsrt(combobox, "dgsComboBoxGetState", 1, "dgs-dxcombobox"))
	end
...
function dgsComboBoxRemoveItem(combobox, i)
	if not dgsIsType(combobox, "dgs-dxcombobox") then
		error(dgsGenAsrt(combobox, "dgsComboBoxRemoveItem", 1, "dgs-dxcombobox"))
	end
	...
	if not (iIsNum and not iNInRange) then
		error(dgsGenAsrt(i, "dgsComboBoxRemoveItem", 2, "number", "1~"..iLen, iNInRange and "Out Of Range"))
	end
...
function dgsComboBoxClear(combobox)
	if not dgsIsType(combobox, "dgs-dxcombobox") then
		error(dgsGenAsrt(combobox, "dgsComboBoxClear", 1, "dgs-dxcombobox"))
	end
...
function dgsComboBoxCreateBox(x, y, w, h, relative, parent)
	...
	if not(type(x) == "number") then
		error(dgsGenAsrt(x, "dgsComboBoxCreateBox", 1, "number"))
	end

	if not(type(y) == "number") then
		error(dgsGenAsrt(y, "dgsComboBoxCreateBox", 2, "number"))
	end

	if not(type(w) == "number") then
		error(dgsGenAsrt(w, "dgsComboBoxCreateBox", 3, "number"))
	end

	if not(type(h) == "number") then
		error(dgsGenAsrt(h, "dgsComboBoxCreateBox", 4, "number"))
	end

	if not(dgsGetType(parent) == "dgs-dxcombobox") then
		error(dgsGenAsrt(parent, "dgsComboBoxCreateBox", 6, "dgs-dxcombobox"))
	end
...
function dgsComboBoxSetSelectedItem(combobox, i)
	if not dgsIsType(combobox, "dgs-dxcombobox") then
		error(dgsGenAsrt(combobox, "dgsComboBoxSetSelectedItem", 1, "dgs-dxcombobox"))
	end
	...
	if not (iIsNum and not iNInRange) then
		error(dgsGenAsrt(i, "dgsComboBoxSetSelectedItem", 2, "number", "-1,1~"..iLen, iNInRange and "Out Of Range"))
	end
...
function dgsComboBoxGetSelectedItem(combobox)
	if not dgsIsType(combobox, "dgs-dxcombobox") then
		error(dgsGenAsrt(combobox, "dgsComboBoxGetSelectedItem", 1, "dgs-dxcombobox"))
	end
...
function dgsComboBoxGetScrollBar(combobox)
	if dgsGetType(combobox) ~= "dgs-dxcombobox" then
		error(dgsGenAsrt(combobox, "dgsComboBoxGetScrollBar", 1, "dgs-dxcombobox"))
	end
...
function dgsComboBoxSetScrollPosition(combobox, vertical)
	if not dgsIsType(combobox, "dgs-dxcombobox") then
		error(dgsGenAsrt(combobox, "dgsComboBoxSetScrollPosition", 1, "dgs-dxcombobox"))
	end

	if vertical and not (type(vertical) == "number" and vertical>= 0 and vertical <= 100) then
		error(dgsGenAsrt(vertical, "dgsComboBoxSetScrollPosition", 2, "nil/number", "0~100"))
	end
...
function dgsComboBoxGetScrollPosition(combobox)
	if not dgsIsType(combobox, "dgs-dxcombobox") then
		error(dgsGenAsrt(combobox, "dgsComboBoxGetScrollPosition", 1, "dgs-dxcombobox"))
	end
...
function dgsComboBoxSetScrollBarState(combobox, state)
	if not dgsIsType(combobox, "dgs-dxcombobox") then
		error(dgsGenAsrt(gridlist, "dgsComboBoxSetScrollBarState", 1, "dgs-dxcombobox"))
	end
...
function dgsComboBoxGetScrollBarState(combobox)
	if not dgsIsType(combobox, "dgs-dxcombobox") then
		error(dgsGenAsrt(combobox, "dgsComboBoxGetScrollBarState", 1, "dgs-dxcombobox"))
	end
...
function dgsComboBoxSetViewCount(combobox, count)
	if not dgsIsType(combobox, "dgs-dxcombobox") then
		error(dgsGenAsrt(combobox, "dgsComboBoxSetViewCount", 1, "dgs-dxcombobox"))
	end

	if not (count == nil or type(count) == "number") then
		error(dgsGenAsrt(count, "dgsComboBoxSetViewCount", 2, "nil/number"))
	end
...
function dgsComboBoxGetViewCount(combobox, count)
	if not dgsIsType(combobox, "dgs-dxcombobox") then
		error(dgsGenAsrt(combobox, "dgsComboBoxGetViewCount", 1, "dgs-dxcombobox"))
	end
...
function dgsComboBoxSetSortFunction(combobox, str)
	if dgsGetType(combobox) ~= "dgs-dxcombobox" then
		error(dgsGenAsrt(combobox, "dgsComboBoxSetSortFunction", 1, "dgs-dxcombobox"))
	end
	...
	if type(str) == "string" then
		if comboSortFunctions[str] then
			...
			if not fnc then
				error("Bad Argument @'dgsComboBoxSetSortFunction' at argument 1, failed to load the function:\n"..err)
			end
		end
...
function dgsComboBoxSortSetAutoSortEnabled(combobox, state)
	if dgsGetType(combobox) ~= "dgs-dxcombobox" then
		error(dgsGenAsrt(combobox, "dgsComboBoxSortSetAutoSortEnabled", 1, "dgs-dxcombobox"))
	end
...
function dgsComboBoxSortGetAutoSortEnabled(combobox)
	if dgsGetType(combobox) ~= "dgs-dxcombobox" then
		error(dgsGenAsrt(combobox, "dgsComboBoxSortGetAutoSortEnabled", 1, "dgs-dxcombobox"))
	end
...
function dgsComboBoxSort(combobox)
	if dgsGetType(combobox) ~= "dgs-dxcombobox" then
		error(dgsGenAsrt(combobox, "dgsComboBoxSort", 1, "dgs-dxcombobox"))
	end
...
dgsRenderer["dgs-dxcombobox-Box"] = function(source, x, y, w, h, mx, my, cx, cy, enabledInherited, enabledSelf, eleData, parentAlpha, isPostGUI, rndtgt, xRT, yRT, xNRT, yNRT, OffsetX, OffsetY, visible)
	...
	if pEleData.bgRT then
		...
		for i = pEleData.FromTo[1], pEleData.FromTo[2] do
			...
			if not textRenderBuffer[textRenderBuffer.count] then
				textRenderBuffer[textRenderBuffer.count] = {}
			end
