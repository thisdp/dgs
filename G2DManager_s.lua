	...
	fetchRemote(targetURL, {}, function(tempData, tempInfo, typ)
		if tempInfo.success then
			...
			while(true) do
				...
				if not liStart_1 or not liEnd_1 then
					break
				end
...
function GenerateNPPAutoComplete(tab)
	if fileExists("nppAC4DGS.xml") then
		fileDelete("nppAC4DGS.xml")
	end
...
function GenerateVSCodeAutoComplete(tab)
	if fileExists("dgs.code-snippets") then
		fileDelete("dgs.code-snippets")
	end
...
function GenerateSublimeAutoComplete(tab)
	if fileExists("dgs.sublime-completions") then
		fileDelete("dgs.sublime-completions")
	end
