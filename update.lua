	...
	for k, v in ipairs(xmlNodeGetChildren(xml)) do
		repeat
			if xmlNodeGetName(v) == "script" or xmlNodeGetName(v) == "file" then
				...
				if string.find(path, "test.lua") and not DGSConfig.testFile then
					break
				end
...
function backupStyleMapper()
	...
	if not fileExists("meta.xml") then
		return outputDGSMessage("Please rename the meta xml as meta.xml", nil, "Updater", 2)
	end
...
function recoverStyleMapper()
	if styleBackupStr == "" then
		return outputDGSMessage("Failed to recover style mapper", "Updater", 2)
	end
