...
function dgsCreateRoundRect(radius, relative, color, texture, colorOverwritten, borderOnlyOrColor, borderThicknessHorizontal, borderThicknessVertical)
	...
	if not(radType == "number" or radType == "table") then
		error(dgsGenAsrt(radius, "dgsCreateRoundRect", 1, "number/table"))
	end
...
function dgsRoundRectSetTexture(rectShader, texture)
	if not(dgsGetPluginType(rectShader) == "dgs-dxroundrectangle") then
		error(dgsGenAsrt(rectShader, "dgsRoundRectSetTexture", 1, "plugin dgs-dxroundrectangle"))
	end

	if isElement(texture) then
		if not(isMaterial(texture) == "texture") then
			error(dgsGenAsrt(texture, "dgsRoundRectSetTexture", 1, "texture"))
		end
...
function dgsRoundRectGetTexture(rectShader)
	if not(dgsGetPluginType(rectShader) == "dgs-dxroundrectangle") then
		error(dgsGenAsrt(rectShader, "dgsRoundRectGetTexture", 1, "plugin dgs-dxroundrectangle"))
	end
...
function dgsRoundRectSetRadius(rectShader, radius, relative)
	if not(dgsGetPluginType(rectShader) == "dgs-dxroundrectangle") then
		error(dgsGenAsrt(rectShader, "dgsRoundRectSetRadius", 1, "plugin dgs-dxroundrectangle"))
	end

	local radType = dgsGetType(radius)

	if not(radType == "number" or radType == "table") then
		error(dgsGenAsrt(radius, "dgsRoundRectSetRadius", 2, "number/table"))
	end
...
function dgsRoundRectGetRadius(rectShader)
	if not(dgsGetPluginType(rectShader) == "dgs-dxroundrectangle") then
		error(dgsGenAsrt(rectShader, "dgsRoundRectGetRadius", 1, "plugin dgs-dxroundrectangle"))
	end
...
function dgsRoundRectSetColor(rectShader, color, secondColor)
	if not(dgsGetPluginType(rectShader) == "dgs-dxroundrectangle") then
		error(dgsGenAsrt(rectShader, "dgsRoundRectSetColor", 1, "plugin dgs-dxroundrectangle"))
	end

	if not(dgsGetType(color) == "number") then
		error(dgsGenAsrt(color, "dgsRoundRectSetColor", 2, "number"))
	end

	if not(dgsGetType(secondColor) == "number" or not secondColor) then
		error(dgsGenAsrt(color, "dgsRoundRectSetColor", 3, "number/nil"))
	end
...
function dgsRoundRectGetColor(rectShader)
	if not(dgsGetPluginType(rectShader) == "dgs-dxroundrectangle") then
		error(dgsGenAsrt(rectShader, "dgsRoundRectGetColor", 1, "plugin dgs-dxroundrectangle"))
	end
...
function dgsRoundRectGetColorOverwritten(rectShader)
	if not(dgsGetPluginType(rectShader) == "dgs-dxroundrectangle") then
		error(dgsGenAsrt(rectShader, "dgsRoundRectGetColorOverwritten", 1, "plugin dgs-dxroundrectangle"))
	end
...
function dgsRoundRectSetColorOverwritten(rectShader, colorOverwritten)
	if not(dgsGetPluginType(rectShader) == "dgs-dxroundrectangle") then
		error(dgsGenAsrt(rectShader, "dgsRoundRectSetColorOverwritten", 1, "plugin dgs-dxroundrectangle"))
	end
...
function dgsRoundRectSetBorderThickness(rectShader, horizontal, vertical)
	...
	if not(dgsGetPluginType(rectShader) == "dgs-dxroundrectangle") then
		error(dgsGenAsrt(rectShader, "dgsRoundRectSetBorderThickness", 1, "plugin dgs-dxroundrectangle"))
	end

	if not(dgsGetType(horizontal) == "number") then
		error(dgsGenAsrt(horizontal, "dgsRoundRectSetBorderThickness", 2, "number"))
	end
...
function dgsRoundRectGetBorderThickness(rectShader)
	if not(dgsGetPluginType(rectShader) == "dgs-dxroundrectangle") then
		error(dgsGenAsrt(rectShader, "dgsRoundRectGetBorderThickness", 1, "plugin dgs-dxroundrectangle"))
	end
...
function dgsRoundRectGetBorderOnly(rectShader)
	if not(dgsGetPluginType(rectShader) == "dgs-dxroundrectangle") then
		error(dgsGenAsrt(rectShader, "dgsRoundRectGetBorderOnly", 1, "plugin dgs-dxroundrectangle"))
	end
