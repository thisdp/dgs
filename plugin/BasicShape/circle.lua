...
function dgsCircleSetRadius(circle, outsideRadius, insideRadius)
	if not(dgsGetPluginType(circle) == "dgs-dxcircle") then
		error(dgsGenAsrt(circle, "dgsCircleSetOutsideRadius", 1, "plugin dgs-dxcircle"))
	end
...
function dgsCircleGetRadius(circle)
	if not(dgsGetPluginType(circle) == "dgs-dxcircle") then
		error(dgsGenAsrt(circle, "dgsCircleGetRadius", 1, "plugin dgs-dxcircle"))
	end
...
function dgsCircleSetTexture(circle, texture)
	if not(dgsGetPluginType(circle) == "dgs-dxcircle") then
		error(dgsGenAsrt(circle, "dgsCircleSetTexture", 1, "plugin dgs-dxcircle"))
	end

	if isElement(texture) then
		if not(isMaterial(texture) == "texture") then
			error(dgsGenAsrt(dgsCircleSetTexture, "dgsCircleSetTexture", 1, "texture"))
		end
...
function dgsCircleGetTexture(circle)
	if not(dgsGetPluginType(circle) == "dgs-dxcircle") then
		error(dgsGenAsrt(circle, "dgsCircleGetTexture", 1, "plugin dgs-dxcircle"))
	end
...
function dgsCircleSetTextureRotation(circle, rot, rotCenterX, rotCenterY)
	if not(dgsGetPluginType(circle) == "dgs-dxcircle") then
		error(dgsGenAsrt(circle, "dgsCircleSetTextureRotation", 1, "plugin dgs-dxcircle"))
	end

	if not(type(rot) == "number") then
		error(dgsGenAsrt(rot, "dgsCircleSetTextureRotation", 2, "number"))
	end
...
function dgsCircleGetTextureRotation(circle)
	if not(dgsGetPluginType(circle) == "dgs-dxcircle") then
		error(dgsGenAsrt(circle, "dgsCircleGetTextureRotation", 1, "plugin dgs-dxcircle"))
	end
...
function dgsCircleSetColor(circle, color)
	if not(dgsGetPluginType(circle) == "dgs-dxcircle") then
		error(dgsGenAsrt(circle, "dgsCircleSetColor", 1, "plugin dgs-dxcircle"))
	end

	if not(type(color) == "number") then
		error(dgsGenAsrt(color, "dgsCircleSetColor", 2, "number"))
	end
...
function dgsCircleGetColor(circle)
	if not(dgsGetPluginType(circle) == "dgs-dxcircle") then
		error(dgsGenAsrt(circle, "dgsCircleGetColor", 1, "plugin dgs-dxcircle"))
	end
...
function dgsCircleSetColorOverwritten(circle, colorOverwritten)
	if not(dgsGetPluginType(circle) == "dgs-dxcircle") then
		error(dgsGenAsrt(circle, "dgsCircleSetColorOverwritten", 1, "plugin dgs-dxcircle"))
	end

	if not(type(colorOverwritten) == "boolean") then
		error(dgsGenAsrt(colorOverwritten, "dgsCircleSetColorOverwritten", 2, "boolean"))
	end
...
function dgsCircleGetColorOverwritten(circle)
	if not(dgsGetPluginType(circle) == "dgs-dxcircle") then
		error(dgsGenAsrt(circle, "dgsCircleGetColorOverwritten", 1, "plugin dgs-dxcircle"))
	end
...
function dgsCircleSetDirection(circle, direction) --true:anticlockwise; false:clockwise
	if not(dgsGetPluginType(circle) == "dgs-dxcircle") then
		error(dgsGenAsrt(circle, "dgsCircleSetDirection", 1, "plugin dgs-dxcircle"))
	end

	if not(type(direction) == "boolean") then
		error(dgsGenAsrt(direction, "dgsCircleSetDirection", 2, "boolean"))
	end
...
function dgsCircleGetDirection(circle)
	if not(dgsGetPluginType(circle) == "dgs-dxcircle") then
		error(dgsGenAsrt(circle, "dgsCircleGetDirection", 1, "plugin dgs-dxcircle"))
	end
...
function dgsCircleSetAngle(circle, angle)
	if not(dgsGetPluginType(circle) == "dgs-dxcircle") then
		error(dgsGenAsrt(circle, "dgsCircleSetAngle", 1, "plugin dgs-dxcircle"))
	end

	if not(type(angle) == "number") then
		error(dgsGenAsrt(angle, "dgsCircleSetAngle", 2, "number"))
	end
...
function dgsCircleGetAngle(circle)
	if not(dgsGetPluginType(circle) == "dgs-dxcircle") then
		error(dgsGenAsrt(circle, "dgsCircleGetAngle", 1, "plugin dgs-dxcircle"))
	end
...
function dgsCircleSetRotation(circle, rot)
	if not(dgsGetPluginType(circle) == "dgs-dxcircle") then
		error(dgsGenAsrt(circle, "dgsCircleSetRotation", 1, "plugin dgs-dxcircle"))
	end

	if not(type(rot) == "number") then
		error(dgsGenAsrt(rot, "dgsCircleSetRotation", 2, "number"))
	end
...
function dgsCircleGetRotation(circle)
	if not(dgsGetPluginType(circle) == "dgs-dxcircle") then
		error(dgsGenAsrt(circle, "dgsCircleGetRotation", 1, "plugin dgs-dxcircle"))
	end
