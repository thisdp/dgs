...
function dgsCreateQuad(vertices, color, texture)
	if type(vertices) ~= "table" then
		error(dgsGenAsrt(vertices, "dgsCreateQuad", 1, "table"))
	end
...
function dgsQuadSetTexture(quad, texture)
	if not(dgsGetPluginType(quad) == "dgs-dxquad") then
		error(dgsGenAsrt(quad, "dgsQuadSetTexture", 1, "plugin dgs-dxquad"))
	end

	if isElement(texture) then
		if not(isMaterial(texture) == "texture") then
			error(dgsGenAsrt(dgsQuadSetTexture, "dgsQuadSetTexture", 1, "texture"))
		end
...
function dgsQuadGetTexture(quad)
	if not(dgsGetPluginType(quad) == "dgs-dxquad") then
		error(dgsGenAsrt(quad, "dgsQuadGetTexture", 1, "plugin dgs-dxquad"))
	end
...
function dgsQuadSetVertices(quad, vertices)
	if not(dgsGetPluginType(quad) == "dgs-dxquad") then
		error(dgsGenAsrt(quad, "dgsQuadSetVertices", 1, "plugin dgs-dxquad"))
	end

	if dgsGetType(vertices) ~= "table" then
		error(dgsGenAsrt(vertices, "dgsQuadSetVertices", 2, "table"))
	end
...
function dgsQuadGetVertices(quad)
	if not(dgsGetPluginType(quad) == "dgs-dxquad") then
		error(dgsGenAsrt(quad, "dgsQuadGetVertices", 1, "plugin dgs-dxquad"))
	end
...
function dgsQuadSetTextureRotation(quad, rot, rotCenterX, rotCenterY)
	if not(dgsGetPluginType(quad) == "dgs-dxquad") then
		error(dgsGenAsrt(quad, "dgsQuadSetTextureRotation", 1, "plugin dgs-dxquad"))
	end

	if not(type(rot) == "number") then
		error(dgsGenAsrt(rot, "dgsQuadSetTextureRotation", 2, "number"))
	end
...
function dgsQuadGetTextureRotation(quad)
	if not(dgsGetPluginType(quad) == "dgs-dxquad") then
		error(dgsGenAsrt(quad, "dgsQuadGetTextureRotation", 1, "plugin dgs-dxquad"))
	end
...
function dgsQuadSetColor(quad, color)
	if not(dgsGetPluginType(quad) == "dgs-dxquad") then
		error(dgsGenAsrt(quad, "dgsQuadSetColor", 1, "plugin dgs-dxquad"))
	end

	if not(type(color) == "number") then
		error(dgsGenAsrt(color, "dgsQuadSetColor", 2, "number"))
	end
...
function dgsQuadGetColor(quad)
	if not(dgsGetPluginType(quad) == "dgs-dxquad") then
		error(dgsGenAsrt(quad, "dgsQuadGetColor", 1, "plugin dgs-dxquad"))
	end
...
function dgsQuadSetColorOverwritten(quad, colorOverwritten)
	if not(dgsGetPluginType(quad) == "dgs-dxquad") then
		error(dgsGenAsrt(quad, "dgsQuadSetColorOverwritten", 1, "plugin dgs-dxquad"))
	end

	if not(type(colorOverwritten) == "boolean") then
		error(dgsGenAsrt(colorOverwritten, "dgsQuadSetColorOverwritten", 2, "boolean"))
	end
...
function dgsQuadGetColorOverwritten(quad)
	if not(dgsGetPluginType(quad) == "dgs-dxquad") then
		error(dgsGenAsrt(quad, "dgsQuadGetColorOverwritten", 1, "plugin dgs-dxquad"))
	end
...
function dgsQuadSetRotation(quad, rot)
	if not(dgsGetPluginType(quad) == "dgs-dxquad") then
		error(dgsGenAsrt(quad, "dgsQuadSetRotation", 1, "plugin dgs-dxquad"))
	end

	if not(type(rot) == "number") then
		error(dgsGenAsrt(rot, "dgsQuadSetRotation", 2, "number"))
	end
...
function dgsQuadGetRotation(quad)
	if not(dgsGetPluginType(quad) == "dgs-dxquad") then
		error(dgsGenAsrt(quad, "dgsQuadGetRotation", 1, "plugin dgs-dxquad"))
	end
