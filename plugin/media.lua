...
function dgsCreateMediaBrowser(w, h, transparent)
	if not(type(w) == "number") then
		error(dgsGenAsrt(w, "dgsCreateMediaBrowser", 1, "number"))
	end

	if not(type(h) == "number") then
		error(dgsGenAsrt(h, "dgsCreateMediaBrowser", 2, "number"))
	end
...
function dgsMediaLoadMedia(media, pathOrData, theType, sourceRes)
	if not(dgsGetType(media) == "dgs-dxmedia") then
		error(dgsGenAsrt(media, "dgsMediaLoadMedia", 1, "plugin dgs-dxmedia"))
	end

	if not(type(pathOrData) == "string") then
		error(dgsGenAsrt(pathOrData, "dgsMediaLoadMedia", 2, "string"))
	end

	local sR = sourceResource or sourceRes or resource
	local name = getResourceName(sR)

	if not(type(theType) == "string") then
		error(dgsGenAsrt(theType, "dgsMediaLoadMedia", 3, "string"))
	end

	theType = theType:upper()

	if not(DGSMediaType[theType]) then
		error(dgsGenAsrt(theType, "dgsMediaLoadMedia", 3, "string", "Audio/Video/Image/ImageData", "unsupported type"))
	end
	...
	if not dgsElementData[media].started then
		...
		if theType == "IMAGE" then
			if not pathOrData:find(":") then
				if pathOrData:sub(1, 1) == "/" then
					pathOrData = pathOrData:sub(2)
				end

				pathOrData = ":"..name.."/"..pathOrData
			end

			if not(fileExists(pathOrData)) then
				error(dgsGenAsrt(pathOrData, "dgsMediaLoadMedia", 2, _, _, "file doesn't exist("..pathOrData..")"))
			end

			dgsSetData(media, "sourcePath", pathOrData)
			str = [[
				var element = document.createElement("]]
..DGSMediaType[theType]..[[");
				element.id = "element";
				element.src = "http://mta/]]
 ..pathOrData:sub(2).. [[";
				element.width = ]]
..size[1]..[[;
				element.height = ]]
..size[2]..[[;
		...
		elseif theType == "IMAGEDATA" then
			...
			if not format or not dgsIsBrowserSupportedImage(format) then
				error(dgsGenAsrt(_, "dgsMediaLoadMedia", 2, _, "unsupported image format ("..tostring(format)..")"))
			end
...
..DGSMediaType[theType]..[[");
				element.id = "element";
				element.src = "]]
..dgsToDataUrl(pathOrData, format)..[[";
				element.width = ]]
..size[1]..[[;
				element.height = ]]
..size[2]..[[;
			...
			if not(fileExists(ppathOrDataath)) then
				error(dgsGenAsrt(pathOrData, "dgsMediaLoadMedia", 2, _, _, "file doesn't exist("..pathOrData..")"))
			end
...
function dgsMediaGetMediaPath(media)
	if not(dgsGetType(media) == "dgs-dxmedia") then
		error(dgsGenAsrt(media, "dgsMediaGetMediaPath", 1, "plugin dgs-dxmedia"))
	end
...
function dgsMediaClearMedia(media)
	if not(dgsGetType(media) == "dgs-dxmedia") then
		error(dgsGenAsrt(media, "dgsMediaClearMedia", 1, "plugin dgs-dxmedia"))
	end
...
function dgsMediaPlay(media)
	if not(dgsGetType(media) == "dgs-dxmedia") then
		error(dgsGenAsrt(media, "dgsMediaPlay", 1, "plugin dgs-dxmedia"))
	end
...
function dgsMediaPause(media)
	if not(dgsGetType(media) == "dgs-dxmedia") then
		error(dgsGenAsrt(media, "dgsMediaPause", 1, "plugin dgs-dxmedia"))
	end
	...
	if not dgsElementData[media].started then
		...
		if not(dgsElementData[media].sourcePath) then
			error(dgsGenAsrt(media, "dgsMediaPause", _, _, _, _, "no media source loaded in dgs-dxmedia"))
		end
...
function dgsMediaStop(media)
	if not(dgsGetType(media) == "dgs-dxmedia") then
		error(dgsGenAsrt(media, "dgsMediaStop", 1, "plugin dgs-dxmedia"))
	end
	...
	if not dgsElementData[media].started then
		...
		if not(dgsElementData[media].sourcePath) then
			error(dgsGenAsrt(media, "dgsMediaStop", _, _, _, _, "no media source loaded in dgs-dxmedia"))
		end
...
function dgsMediaSetSpeed(media, speed)
	if not(dgsGetType(media) == "dgs-dxmedia") then
		error(dgsGenAsrt(media, "dgsMediaSetSpeed", 1, "plugin dgs-dxmedia"))
	end
	...
	if not dgsElementData[media].started then
		...
		if not(dgsElementData[media].sourcePath) then
			error(dgsGenAsrt(media, "dgsMediaSetSpeed", _, _, _, _, "no media source loaded in dgs-dxmedia"))
		end
...
function dgsMediaGetSpeed(media, speed)
	if not(dgsGetType(media) == "dgs-dxmedia") then
		error(dgsGenAsrt(media, "dgsMediaGetSpeed", 1, "plugin dgs-dxmedia"))
	end
...
function dgsMediaSetFullScreen(media, state)
	if not(dgsGetType(media) == "dgs-dxmedia") then
		error(dgsGenAsrt(media, "dgsMediaSetFullScreen", 1, "plugin dgs-dxmedia"))
	end
...
function dgsMediaGetFullScreen(media)
	if not(dgsGetType(media) == "dgs-dxmedia") then
		error(dgsGenAsrt(media, "dgsMediaGetFullScreen", 1, "plugin dgs-dxmedia"))
	end
...
function dgsMediaSetFilled(media, state)
	if not(dgsGetType(media) == "dgs-dxmedia") then
		error(dgsGenAsrt(media, "dgsMediaSetFilled", 1, "plugin dgs-dxmedia"))
	end
...
function dgsMediaGetFilled(media)
	if not(dgsGetType(media) == "dgs-dxmedia") then
		error(dgsGenAsrt(media, "dgsMediaGetFilled", 1, "plugin dgs-dxmedia"))
	end
...
function dgsMediaGetLooped(media)
	if not(dgsGetType(media) == "dgs-dxmedia") then
		error(dgsGenAsrt(media, "dgsMediaGetLooped", 1, "plugin dgs-dxmedia"))
	end
...
function dgsMediaSetLooped(media, state)
	if not(dgsGetType(media) == "dgs-dxmedia") then
		error(dgsGenAsrt(media, "dgsMediaSetLooped", 1, "plugin dgs-dxmedia"))
	end
...
function dgsMediaSetSize(media, w, h)
	if not(dgsGetType(media) == "dgs-dxmedia") then
		error(dgsGenAsrt(media, "dgsMediaSetSize", 1, "plugin dgs-dxmedia"))
	end

	if not(type(w) == "number" and w > 0) then
		error(dgsGenAsrt(w, "dgsMediaSetSize", 2, "number", ">0"))
	end

	if not(type(h) == "number" and h > 0) then
		error(dgsGenAsrt(h, "dgsMediaSetSize", 3, "number", ">0"))
	end
...
function dgsMediaGetDuration(media)
	if not(dgsGetType(media) == "dgs-dxmedia") then
		error(dgsGenAsrt(media, "dgsMediaGetDuration", 1, "plugin dgs-dxmedia"))
	end
...
function dgsMediaGetCurrentPosition(media)
	if not(dgsGetType(media) == "dgs-dxmedia") then
		error(dgsGenAsrt(media, "dgsMediaGetCurrentPosition", 1, "plugin dgs-dxmedia"))
	end
...
function dgsMediaSetCurrentPosition(media, position) --Failed to Set current position ( CEF doesn't load the video buffer )
	if not(dgsGetType(media) == "dgs-dxmedia") then
		error(dgsGenAsrt(media, "dgsMediaSetCurrentPosition", 1, "plugin dgs-dxmedia"))
	end

	if not(type(position) == "number" and position > 0) then
		error(dgsGenAsrt(position, "dgsMediaSetCurrentPosition", 2, "number", ">0"))
	end
	...
	if not dgsElementData[media].started then
		...
		if not(dgsElementData[media].sourcePath) then
			error(dgsGenAsrt(media, "dgsMediaSetCurrentPosition", _, _, _, _, "no media source loaded in dgs-dxmedia"))
		end
