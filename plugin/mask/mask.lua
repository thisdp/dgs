...
function dgsCreateMask(texture1, texture2, settings)
	...
	if not(isMaterial(texture1) == "texture") then
		error(dgsGenAsrt(texture1, "dgsCreateMask", 1, "texture"))
	end
	...
	if tex2Type == "string" then
		if not(masks[texture2]) then
			error(dgsGenAsrt(texture2, "dgsCreateMask", 2, "texture", _, _, "unsupported type"))
		end
...
function dgsMaskSetTexture(mask, texture)
	if not(dgsGetPluginType(mask) == "dgs-dxmask") then
		error(dgsGenAsrt(mask, "dgsMaskSetTexture", 1, "dgs-dxmask"))
	end
...
function dgsMaskGetTexture(mask)
	if not(dgsGetPluginType(mask) == "dgs-dxmask") then
		error(dgsGenAsrt(mask, "dgsMaskGetTexture", 1, "dgs-dxmask"))
	end
...
function dgsMaskGetSetting(mask, settingName)
	if not(dgsGetPluginType(mask) == "dgs-dxmask") then
		error(dgsGenAsrt(mask, "dgsMaskGetSetting", 1, "dgs-dxmask"))
	end
...
function dgsMaskSetSetting(mask, settingName, value)
	if not(dgsGetPluginType(mask) == "dgs-dxmask") then
		error(dgsGenAsrt(mask, "dgsMaskSetSetting", 1, "dgs-dxmask"))
	end
...
function dgsMaskCenterTexturePosition(mask, w, h)
	if not(dgsGetPluginType(mask) == "dgs-dxmask") then
		error(dgsGenAsrt(mask, "dgsMaskCenterTexturePosition", 1, "dgs-dxmask"))
	end
...
function dgsMaskAdaptTextureSize(mask, w, h)
	if not(dgsGetPluginType(mask) == "dgs-dxmask") then
		error(dgsGenAsrt(mask, "dgsMaskAdaptTextureSize", 1, "dgs-dxmask"))
	end
