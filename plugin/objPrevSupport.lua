...
function dgsLocateObjectPreviewResource(name)
	if not(type(name) == "string") then
		error(dgsGenAsrt(name, "dgsLocateObjectPreviewResource", 1, "string"))
	end

	objPrevResStatus.res = getResourceFromName(name)

	if not(objPrevResStatus.res) then
		error(dgsGenAsrt(name, "dgsLocateObjectPreviewResource", 1, _, _, _, "resource "..name.." is not running or doesn't exist"))
	end

	objPrevResStatus.name = name

	if not(getResourceState(objPrevResStatus.res) == "running") then
		error(dgsGenAsrt(name, "dgsLocateObjectPreviewResource", 1, _, _, _, "resource "..name.." is not running, please start it"))
	end
...
function dgsCreateObjectPreviewHandle(objEle, rX, rY, rZ)
	if not(objPrevResStatus.name ~= "") then
		error(dgsGenAsrt(objEle, "dgsCreateObjectPreviewHandle", _, _, _, _, "couldn't find Object Preview resource, please locate it"))
	end

	if not(isElement(objEle)) then
		error(dgsGenAsrt(objEle, "dgsCreateObjectPreviewHandle", 1, "element"))
	end
...
function dgsAttachObjectPreviewToImage(objPrev, dgsImage)
	if not(dgsGetPluginType(objPrev) == "dgs-dxobjectpreviewhandle") then
		error(dgsGenAsrt(objPrev, "dgsAttachObjectPreviewToImage", 1, "dgs-dxobjectpreviewhandle"))
	end

	if not(dgsGetType(dgsImage) == "dgs-dximage") then
		error(dgsGenAsrt(dgsImage, "dgsAttachObjectPreviewToImage", 2, "dgs-dximage"))
	end
...
function dgsRemoveObjectPreviewFromImage(dgsImage)
	if not(dgsGetType(dgsImage) == "dgs-dximage") then
		error(dgsGenAsrt(dgsImage, "dgsRemoveObjectPreviewFromImage", 1, "dgs-dximage"))
	end
