...
function dgsCreateBlurBox(w, h, sourceTexture, level)
	if not(type(w) == "number") then
		error(dgsGenAsrt(w, "dgsCreateBlurBox", 1, "number"))
	end

	if not(type(h) == "number") then
		error(dgsGenAsrt(h, "dgsCreateBlurBox", 2, "number"))
	end
...
function dgsBlurBoxSetFilter(bb, shader)
	if not(dgsGetPluginType(bb) == "dgs-dxblurbox") then
		error(dgsGenAsrt(bb, "dgsBlurBoxSetResolution", 1, "dgs-dxblurbox"))
	end
...
function dgsBlurBoxSetResolution(bb, w, h)
	...
	if not(type(w) == "number") then
		error(dgsGenAsrt(w, "dgsBlurBoxSetResolution", 2, "number"))
	end

	if not(type(h) == "number") then
		error(dgsGenAsrt(h, "dgsBlurBoxSetResolution", 3, "number"))
	end
	...
	if isElement(bufferRTH) then
		destroyElement(bufferRTH)
	end

	if isElement(bufferRTV) then
		destroyElement(bufferRTV)
	end
...
function dgsBlurBoxSetIntensity(bb, intensity)
	if not(dgsGetPluginType(bb) == "dgs-dxblurbox") then
		error(dgsGenAsrt(bb, "dgsBlurBoxSetIntensity", 1, "dgs-dxblurbox"))
	end

	if not(type(intensity) == "number") then
		error(dgsGenAsrt(intensity, "dgsBlurBoxSetIntensity", 2, "number"))
	end
...
function dgsBlurBoxSetLevel(bb, level)
	if not(dgsGetPluginType(bb) == "dgs-dxblurbox") then
		error(dgsGenAsrt(bb, "dgsBlurBoxSetLevel", 1, "dgs-dxblurbox"))
	end

	local inRange = level>=0 and level <=15

	if not(type(level) == "number" and inRange) then
		error(dgsGenAsrt(level, "dgsBlurBoxSetLevel", 2, "number", "0~15", not inRange and "Out of range"))
	end
...
function dgsBlurBoxGetResolution(bb)
	if not(dgsGetPluginType(bb) == "dgs-dxblurbox") then
		error(dgsGenAsrt(bb, "dgsBlurBoxGetResolution", 1, "dgs-dxblurbox"))
	end
...
function dgsBlurBoxGetIntensity(bb)
	if not(dgsGetPluginType(bb) == "dgs-dxblurbox") then
		error(dgsGenAsrt(bb, "dgsBlurBoxGetIntensity", 1, "dgs-dxblurbox"))
	end
...
function dgsBlurBoxGetLevel(bb)
	if not(dgsGetPluginType(bb) == "dgs-dxblurbox") then
		error(dgsGenAsrt(bb, "dgsBlurBoxGetLevel", 1, "dgs-dxblurbox"))
	end
...
function dgsBlurBoxSetBrightness(bb, brt)
	if not(type(brt) == "number") then
		error(dgsGenAsrt(brt, "dgsBlurBoxSetBrightness", 2, "number"))
	end

	if not(dgsGetPluginType(bb) == "dgs-dxblurbox") then
		error(dgsGenAsrt(bb, "dgsBlurBoxSetBrightness", 1, "dgs-dxblurbox"))
	end
...
function dgsBlurBoxGetBrightness(bb)
	if not(dgsGetPluginType(bb) == "dgs-dxblurbox") then
		error(dgsGenAsrt(bb, "dgsBlurBoxGetBrightness", 1, "dgs-dxblurbox"))
	end
...
function dgsBlurBoxSetTexture(bb, texture)
	if not(dgsGetPluginType(bb) == "dgs-dxblurbox") then
		error(dgsGenAsrt(bb, "dgsBlurBoxSetTexture", 1, "plugin dgs-dxblurbox"))
	end
...
function dgsBlurBoxGetTexture(bb)
	if not(dgsGetPluginType(bb) == "dgs-dxblurbox") then
		error(dgsGenAsrt(bb, "dgsBlurBoxGetTexture", 1, "plugin dgs-dxblurbox"))
	end
