...
function dgsCreateNineSlice(texture, gridXLeft, gridXRight, gridYTop, gridYBottom, relative)
	...
	if not(imgType == "texture" or imgType == "svg") then
		error(dgsGenAsrt(texture, "dgsCreateNineSlice", 1, "texture"))
	end

	if not(type(gridXLeft) == "number") then
		error(dgsGenAsrt(gridXLeft, "dgsCreateNineSlice", 2, "number"))
	end

	if not(type(gridXRight) == "number") then
		error(dgsGenAsrt(gridXRight, "dgsCreateNineSlice", 3, "number"))
	end

	if not(type(gridYTop) == "number") then
		error(dgsGenAsrt(gridYTop, "dgsCreateNineSlice", 4, "number"))
	end

	if not(type(gridYBottom) == "number") then
		error(dgsGenAsrt(gridYBottom, "dgsCreateNineSlice", 5, "number"))
	end
...
function dgsNineSliceSetGrid(nineSlice, gridXLeft, gridXRight, gridYTop, gridYBottom, relative)
	...
	if not(dgsGetPluginType(nineSlice) == "dgs-dxnineslice") then
		error(dgsGenAsrt(nineSlice, "dgsNineSliceSetGrid", 1, "dgs-dxnineslice"))
	end
...
function dgsNineSliceGetGrid(nineSlice, relative)
	if not(dgsGetPluginType(nineSlice) == "dgs-dxnineslice") then
		error(dgsGenAsrt(nineSlice, "dgsNineSliceGetGrid", 1, "dgs-dxnineslice"))
	end
...
function dgsNineSliceSetTexture(nineSlice, texture)
	if not(dgsGetPluginType(nineSlice) == "dgs-dxnineslice") then
		error(dgsGenAsrt(nineSlice, "dgsNineSliceSetTexture", 1, "dgs-dxnineslice"))
	end

	local imgType = dgsGetType(texture)

	if not(imgType == "texture" or imgType == "svg") then
		error(dgsGenAsrt(texture, "dgsNineSliceSetTexture", 2, "texture"))
	end
...
function dgsNineSliceGetTexture(nineSlice)
	if not(dgsGetPluginType(nineSlice) == "dgs-dxnineslice") then
		error(dgsGenAsrt(nineSlice, "dgsNineSliceGetTexture", 1, "dgs-dxnineslice"))
	end
