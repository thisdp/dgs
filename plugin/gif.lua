...
local function tryYield()
    if yieldEnabled and coroutine.running() then
        coroutine.yield()
    end
end
...
local function lzwDecode(minCodeSize, data, expectedSize)
    ...
    while true do
        ...
        if expectedSize and outLen >= expectedSize then
            break
        end
    end
...
function dgsGIFDecode(input, maxFrames)
    ...
    if type(input) ~= "string" then
        error(dgsGenAsrt(input, "dgsGIFDecode", 1, "string or binary string"))
    end
    ...
    local pos = 1
    local header = raw:sub(pos, pos+5);

    pos = pos + 6

    if header ~= "GIF87a" and header ~= "GIF89a" then
        error("Unsupported GIF header")
    end

    local width = readLE16(raw, pos);
    pos = pos + 2
    local height = readLE16(raw, pos);
    pos = pos + 2
    local packed = strByte(raw, pos);
    pos = pos + 1
    local gctFlag = mathFloor(packed / 128) % 2 == 1
    local gctSize = 2 ^ ((packed % 8) + 1)
    local bgIndex = strByte(raw, pos);

    pos = pos + 1
    local pixelAspect = strByte(raw, pos);
    pos = pos + 1
    ...
    local function readSubBlocks()
        ...
        while true do
            local blockSize = strByte(raw, pos);
            pos = pos + 1
    ...
    while pos <= #raw do
        ...
        if b == 0x2C then
            local left = readLE16(raw, pos);
            pos = pos + 2
            local top = readLE16(raw, pos);
            pos = pos + 2
            local iw = readLE16(raw, pos);
            pos = pos + 2
            local ih = readLE16(raw, pos);
            pos = pos + 2
            local ipacked = strByte(raw, pos);
            pos = pos + 1
            ...
            if lctFlag then
                palette, pos = parseColorTable(raw, pos, lctSize)
            end

            local lzwMin = strByte(raw, pos);
            pos = pos + 1
...
local function ensureDecodedUpTo(gif, target)
    ...
    for i = decodedUpTo + 1, target do
        ...
        elseif meta.gce and meta.gce.disposal == 3 then
            if prevBackup then
                restoreRegion(prevCanvas, prevBackup, width)
            end
        end
...
function dgsCreateGIF(pathOrData, lazyLoad)
    if type(pathOrData) ~= "string" then
        error(dgsGenAsrt(pathOrData, "dgsCreateGIF", 1, "string or binary data"))
    end

    if lazyLoad == nil then
        lazyLoad = true
    end
...
function dgsGIFGetSize(gif)
    if not dgsGetPluginType(gif) == "dgs-dxgif" then
        error(dgsGenAsrt(gif, "dgsGIFGetSize", 1, "plugin dgs-dxgif"))
    end
...
function dgsGIFGetImageCount(gif)
    if not dgsGetPluginType(gif) == "dgs-dxgif" then
        error(dgsGenAsrt(gif, "dgsGIFGetImageCount", 1, "plugin dgs-dxgif"))
    end
...
function dgsGIFGetImages(gif)
    if not dgsGetPluginType(gif) == "dgs-dxgif" then
        error(dgsGenAsrt(gif, "dgsGIFGetImages", 1, "plugin dgs-dxgif"))
    end
...
function dgsGIFPlay(gif, speed, frame)
    if not isElement(gif) or dgsGetPluginType(gif) ~= "dgs-dxgif" then
        error(dgsGenAsrt(gif, "dgsGIFPlay", 1, "plugin dgs-dxgif"))
    end
...
function dgsGIFStop(gif)
    if not isElement(gif) or dgsGetPluginType(gif) ~= "dgs-dxgif" then
        error(dgsGenAsrt(gif, "dgsGIFStop", 1, "plugin dgs-dxgif"))
    end
...
function dgsGIFSetSpeed(gif, speed)
    if not isElement(gif) or dgsGetPluginType(gif) ~= "dgs-dxgif" then
        error(dgsGenAsrt(gif, "dgsGIFSetSpeed", 1, "plugin dgs-dxgif"))
    end
...
function dgsGIFGetSpeed(gif)
    if not isElement(gif) or dgsGetPluginType(gif) ~= "dgs-dxgif" then
        error(dgsGenAsrt(gif, "dgsGIFGetSpeed", 1, "plugin dgs-dxgif"))
    end
...
function dgsGIFGetPlaying(gif)
    if not isElement(gif) or dgsGetPluginType(gif) ~= "dgs-dxgif" then
        error(dgsGenAsrt(gif, "dgsGIFGetPlaying", 1, "plugin dgs-dxgif"))
    end
...
function dgsGIFSetLooped(gif, looped)
    if not isElement(gif) or dgsGetPluginType(gif) ~= "dgs-dxgif" then
        error(dgsGenAsrt(gif, "dgsGIFSetLooped", 1, "plugin dgs-dxgif"))
    end
...
function dgsGIFGetLooped(gif)
    if not isElement(gif) or dgsGetPluginType(gif) ~= "dgs-dxgif" then
        error(dgsGenAsrt(gif, "dgsGIFGetLooped", 1, "plugin dgs-dxgif"))
    end
...
function dgsGIFSetFrameID(gif, frame)
    if not isElement(gif) or dgsGetPluginType(gif) ~= "dgs-dxgif" then
        error(dgsGenAsrt(gif, "dgsGIFSetFrameID", 1, "plugin dgs-dxgif"))
    end
...
function dgsGIFGetFrameID(gif)
    if not isElement(gif) or dgsGetPluginType(gif) ~= "dgs-dxgif" then
        error(dgsGenAsrt(gif, "dgsGIFGetFrameID", 1, "plugin dgs-dxgif"))
    end
...
local function processDecodeQueue()
    ...
    for gif, entry in pairs(decodeQueue) do
        ...
        if getTickCount() - start >= DECODE_BUDGET_MS then
            break
        end
    end
...
function scheduleEnsureDecodedUpTo(gif, target)
    ...
    if target <= (data.decodedUpTo or 0) then
        return
    end
