...
-- Pure-Lua GIF decoder (client-side), stores frames in memory and creates textures.
-- Usage:
-- local gif = dgsCreateGIF(gifPath)
-- local w,h = dgsGIFGetMaterialSize(gif)
-- local imageCount = dgsGIFGetImageCount(gif)
-- local images = dgsGIFGetImages(gif) -- array of texture elements
...
-- Decode GIF file and return frames (textures), delays (in hundredths), width, height
function dgsGIFDecode(input, maxFrames)
    ...
    while pos <= #raw do
        ...
        if b == 0x2C then -- Image Descriptor
            ...
            local indices = lzwDecode(lzwMin, imgData, iw * ih)
            if interlaced then indices = deinterlace(indices, iw, ih) end

            local prevBackup
            if gce.disposal == 3 then prevBackup = backupRegion(prevCanvas, left, top, iw, ih, width, height) end

            local frameCanvas = {}
            for i=1, width*height do frameCanvas[i] = {prevCanvas[i][1], prevCanvas[i][2], prevCanvas[i][3], prevCanvas[i][4]} end
            local idx = 1
            for outY=top, ih-1+top do
                if outY >=0 and outY < height then
                    local outYxWidth = outY * width+1
                    for outX=left, iw-1+left do
                        if outX >=0 and outX < width then
                            local paletteIndex = indices[idx]
                            if not (gce.transparent and paletteIndex == gce.transIndex) then
                                local col = palette[paletteIndex]
                                if col then
            ...
            if gce.disposal == 2 then
                for y=top, top+ih-1 do
                    for x=left, left+iw-1 do
            ...
            elseif gce.disposal == 3 then
                if prevBackup then restoreRegion(prevCanvas, prevBackup) end
            else
...
function dgsGIFPlay(gif, speed, frame)
    ...
    local frames = dgsElementData[gif].frames or {}
    ...
    local cur = (type(frame)=="number" and mathClamp(frame,1,#frames)) or dgsElementData[gif].currentFrame or 1
    cur = mathClamp(cur, 1, #frames)
...
function dgsGIFSetFrameID(gif, frame)
    ...
    local frames = dgsElementData[gif].frames or {}
    frame = frame-frame%1
    frame = mathClamp(frame, 1, #frames)
    -- compute pausedElapsed as cumulative time up to frame-1
    local delays = dgsElementData[gif].delays or {}
    local sum = 0
...
-- Register custom texture renderer for GIF plugin
dgsCustomTexture["dgs-dxgif"] = function(posX,posY,width,height,u,v,usize,vsize,image,rotation,rotationX,rotationY,color,postGUI,isInRndTgt)
    local frames = dgsElementData[image].frames or {}
    if #frames == 0 then return end
    local idx
    if dgsElementData[image].playing then
        idx = dgsGIFCalculateCurrentFrameID(image)
        dgsSetData(image,'currentFrame', idx)
    else
        idx = dgsElementData[image].currentFrame or 1
    end
    idx = mathClamp(idx, 1, #frames)
    local tex = frames[idx]
    if not isElement(tex) then return end
    -- draw current frame texture
