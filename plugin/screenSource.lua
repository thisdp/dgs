...
function dgsScreenSourceSetUVPosition(ss, uPos, vPos, relative)
	if not(dgsGetPluginType(ss) == "dgs-dxscreensource") then
		error(dgsGenAsrt(ss, "dgsScreenSourceSetUVPosition", 1, "dgs-dxscreensource"))
	end
...
function dgsScreenSourceSetUVSize(ss, uSize, vSize, relative)
	if not(dgsGetPluginType(ss) == "dgs-dxblurbox") then
		error(dgsGenAsrt(ss, "dgsScreenSourceSetUVSize", 1, "dgs-dxblurbox"))
	end
...
function dgsScreenSourceGetUVSize(ss, relative)
	if not(dgsGetPluginType(ss) == "dgs-dxscreensource") then
		error(dgsGenAsrt(ss, "dgsScreenSourceGetUVSize", 1, "dgs-dxscreensource"))
	end
