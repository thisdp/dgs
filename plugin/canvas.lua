...
function dgsCreateCanvas(renderSource, w, h)
	if not(isMaterial(renderSource) or dgsGetType(renderSource) == "dgs-dxcustomrenderer") then
		error(dgsGenAsrt(renderSource, "dgsCreateCanvas", 1, "material/dgs-dxcustomrenderer"))
	end

	if not(type(w) == "number") then
		error(dgsGenAsrt(w, "dgsCreateCanvas", 2, "number"))
	end

	if not(type(h) == "number") then
		error(dgsGenAsrt(h, "dgsCreateCanvas", 3, "number"))
	end
...
function dgsCanvasSetBackEndFunction(canvas, fncStr)
	if dgsGetType(canvas) ~= "dgs-dxcanvas" then
		error(dgsGenAsrt(canvas, "dgsCanvasSetBackEndFunction", 1, "dgs-dxcanvas"))
	end
	...
	if fncType == "function" then
		...
		if not (type(fncStr) == "string") then
			error(dgsGenAsrt(fncStr, "dgsCanvasSetBackEndFunction", 2, "string"))
		end
		...
		if not fnc then
			error(dgsGenAsrt(fnc, "dgsCanvasSetBackEndFunction", 2, _, _, _, "Failed to load function:"..err))
		end
