...
function dgsCreateColorPicker(style, ...)
	if not(type(style) == "string") then
		error(dgsGenAsrt(style, "dgsCreateColorPicker", 1, "string"))
	end
...
function dgsComponentSelectorSetCursorThickness(cs, thickness)
	if not(dgsGetPluginType(cs) == "dgs-dxcomponentselector") then
		error(dgsGenAsrt(cs, "dgsComponentSelectorSetCursorThickness", 1, "plugin dgs-dxcomponentselector"))
	end
...
function dgsComponentSelectorGetCursorThickness(cs)
	if not(dgsGetPluginType(cs) == "dgs-dxcomponentselector") then
		error(dgsGenAsrt(cs, "dgsComponentSelectorGetCursorThickness", 1, "plugin dgs-dxcomponentselector"))
	end
...
function dgsColorPickerSetComponentSelectorMask(cs, maskTexture)
	if not(dgsGetPluginType(cs) == "dgs-dxcomponentselector") then
		error(dgsGenAsrt(cs, "dgsColorPickerSetComponentSelectorMask", 1, "plugin dgs-dxcomponentselector"))
	end
...
function dgsColorPickerGetComponentSelectorMask(cs)
	if not(dgsGetPluginType(cs) == "dgs-dxcomponentselector") then
		error(dgsGenAsrt(cs, "dgsColorPickerGetComponentSelectorMask", 1, "plugin dgs-dxcomponentselector"))
	end
...
function dgsColorPickerGetComponentSelectorValue(cs)
	if not(dgsGetPluginType(cs) == "dgs-dxcomponentselector") then
		error(dgsGenAsrt(cs, "dgsColorPickerGetComponentSelectorValue", 1, "plugin dgs-dxcomponentselector"))
	end
...
function dgsColorPickerSetComponentSelectorValue(cs, value)
	if not(dgsGetPluginType(cs) == "dgs-dxcomponentselector") then
		error(dgsGenAsrt(cs, "dgsColorPickerSetComponentSelectorValue", 1, "plugin dgs-dxcomponentselector"))
	end

	if not(type(value) == "number") then
		error(dgsGenAsrt(value, "dgsColorPickerSetComponentSelectorValue", 2, "number"))
	end
...
updateSelectorOnColorChange_ComponentSelector = function()
	...
	if dgsElementData[source].setCool then
		return
	end

	local cp = dgsElementData[source].bindColorPicker

	if not isElement(cp) then
		return dgsUnbindFromColorPicker(source)
	end
...
function dgsBindToColorPicker(cs, colorPicker, colorType, colorAttribute, staticMode, isReversed)
	if not(dgsIsType(cs)) then
		error(dgsGenAsrt(cs, "dgsBindToColorPicker", 1, "dgs-dxgui"))
	end

	if not(dgsGetPluginType(colorPicker) == "dgs-dxcolorpicker") then
		error(dgsGenAsrt(colorPicker, "dgsBindToColorPicker", 2, "plugin dgs-dxcolorpicker"))
	end

	if colorAttribute ~= "A" and colorType ~= "#RGBAHEX" and colorType ~= "RGBAHEX"  then
		if not(AvailableColorType[colorType]) then
			error(dgsGenAsrt(colorType, "dgsBindToColorPicker", 3, "string", "RGB/HSL/HSV/#RGBAHEX/RGBAHEX"))
		end

		if not(AvailableColorType[colorType][colorAttribute]) then
			error(dgsGenAsrt(colorAttribute, "dgsBindToColorPicker", 4, "string", table.concat(ColorAttributeOrder[colorType], "/")))
		end
	end
...
function dgsUnbindFromColorPicker(cs)
	if not(dgsIsType(cs)) then
		error(dgsGenAsrt(cs, "dgsUnbindFromColorPicker", 1, "dgs-dxgui"))
	end
...
function dgsColorPickerSetColor(cp, ...)
	if not(dgsGetPluginType(cp) == "dgs-dxcolorpicker") then
		error(dgsGenAsrt(cp, "dgsColorPickerSetColor", 1, "plugin dgs-dxcolorpicker"))
	end
...
function dgsColorPickerGetColor(cp, mode)
	if not(dgsGetPluginType(cp) == "dgs-dxcolorpicker") then
		error(dgsGenAsrt(cp, "dgsColorPickerGetColor", 1, "plugin dgs-dxcolorpicker"))
	end
