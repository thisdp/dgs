...
function dgsCreateCmd(x, y, w, h, relative, parent)
	if not(type(x) == "number") then
		error(dgsGenAsrt(x, "dgsCreateCmd", 1, "number"))
	end

	if not(type(y) == "number") then
		error(dgsGenAsrt(y, "dgsCreateCmd", 2, "number"))
	end

	if not(type(w) == "number") then
		error(dgsGenAsrt(w, "dgsCreateCmd", 3, "number"))
	end

	if not(type(h) == "number") then
		error(dgsGenAsrt(h, "dgsCreateCmd", 4, "number"))
	end
...
function dgsCmdApplyDefaultCommands(cmd)
	if not(dgsGetPluginType(cmd) == "dgs-dxcmd") then
		error(dgsGenAsrt(cmd, "dgsCmdApplyDefaultCommands", 1, "plugin dgs-dxcmd"))
	end
...
function dgsCmdClearText(cmd)
	if not(dgsGetPluginType(cmd) == "dgs-dxcmd") then
		error(dgsGenAsrt(cmd, "dgsCmdClearText", 1, "plugin dgs-dxcmd"))
	end
...
function dgsCmdIsInWhiteList(cmd, rule)
	if not(dgsGetPluginType(cmd) == "dgs-dxcmd" or cmd == "all") then
		error(dgsGenAsrt(cmd, "dgsCmdIsInWhiteList", 1, "plugin dgs-dxcmd/string", "all"))
	end

	if not(type(rule) == "string") then
		error(dgsGenAsrt(rule, "dgsCmdIsInWhiteList", 2, "string"))
	end
...
function outputCmdMessage(cmd, str)
	if not(dgsGetPluginType(cmd) == "dgs-dxcmd") then
		error(dgsGenAsrt(cmd, "outputCmdMessage", 1, "plugin dgs-dxcmd"))
	end
...
function receiveCmdEditInput(cmd, str)
	if not(dgsGetPluginType(cmd) == "dgs-dxcmd") then
		error(dgsGenAsrt(cmd, "receiveCmdEditInput", 1, "plugin dgs-dxcmd"))
	end
...
function dgsCmdAddCommandHandler(...)
	if select("#", ...) == 2 then
		...
		if not(type(str) == "string") then
			error(dgsGenAsrt(str, "dgsCmdAddCommandHandler", 1, "string"))
		end

		if not(type(func) == "function") then
			error(dgsGenAsrt(func, "dgsCmdAddCommandHandler", 2, "function"))
		end
	...
	elseif select("#", ...) == 3 then
		...
		if not(type(str) == "string") then
			error(dgsGenAsrt(str, "dgsCmdAddCommandHandler", 2, "string"))
		end

		if not(type(func) == "function") then
			error(dgsGenAsrt(func, "dgsCmdAddCommandHandler", 3, "function"))
		end
...
function dgsCmdRemoveCommandHandler(...)
	if select("#", ...) == 2 then
		...
		if not(type(str) == "string") then
			error(dgsGenAsrt(str, "dgsCmdRemoveCommandHandler", 1, "string"))
		end

		if not(type(func) == "function") then
			error(dgsGenAsrt(func, "dgsCmdRemoveCommandHandler", 2, "function"))
		end
	...
	elseif select("#", ...) == 3 then
		...
		if not(type(str) == "string") then
			error(dgsGenAsrt(str, "dgsCmdRemoveCommandHandler", 2, "string"))
		end

		if not(type(func) == "function") then
			error(dgsGenAsrt(func, "dgsCmdRemoveCommandHandler", 3, "function"))
		end
...
function dgsBuildInCMD(command)
	if not getElementData(resourceRoot, "DGS-allowCMD") then
		return outputChatBox("[DGS]Access Denied", 255, 0, 0)
	end
...
function dxStatusUpdate()
	if not isElement(dxStatus["dxList"]) then
		return removeEventHandler("onClientRender", root, dxStatusUpdate)
	end
