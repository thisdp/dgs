...
function dgsCreateRemoteImage(website)
	...
	addEventHandler("onClientElementDestroy", remoteImage, function()
		...
		if not isElement(textureRef) then
			return
		end
...
function dgsRemoteImageRequest(remoteImage, website, forceReload)
	if not(dgsGetPluginType(remoteImage) == "dgs-dxremoteimage") then
		error(dgsGenAsrt(remoteImage, "dgsRemoteImageRequest", 1, "plugin dgs-dxremoteimage"))
	end

	if not(type(website) == "string") then
		error(dgsGenAsrt(remoteImage, "dgsRemoteImageRequest", 2, "string"))
	end
...
function dgsRemoteImageDoRequest(remoteImage, website)
	if not(dgsGetPluginType(remoteImage) == "dgs-dxremoteimage") then
		error(dgsGenAsrt(remoteImage, "dgsRemoteImageDoRequest", 1, "plugin dgs-dxremoteimage"))
	end

	if not(type(website) == "string") then
		error(dgsGenAsrt(remoteImage, "dgsRemoteImageDoRequest", 2, "string"))
	end
...
function dgsRemoteImageAbort(remoteImage)
	if not(dgsGetPluginType(remoteImage) == "dgs-dxremoteimage") then
		error(dgsGenAsrt(remoteImage, "dgsRemoteImageAbort", 1, "plugin dgs-dxremoteimage"))
	end

	local fetchRemoteHandler = dgsElementData[remoteImage].fetchRemoteHandler

	if not fetchRemoteHandler then
		return true
	end
...
function dgsGetRemoteImageLoadState(remoteImage)
	if not(dgsGetPluginType(remoteImage) == "dgs-dxremoteimage") then
		error(dgsGenAsrt(remoteImage, "dgsGetRemoteImageLoadState", 1, "plugin dgs-dxremoteimage"))
	end
