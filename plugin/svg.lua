...
function dgsCreateSVG(...)
	...
	if select('#', ...) == 3 then
		...
		if not(type(w) == "number") then
			error(dgsGenAsrt(w, "dgsCreateSVG", 1, "number"))
		end

		if not(type(h) == "number") then
			error(dgsGenAsrt(h, "dgsCreateSVG", 2, "number"))
		end

		if not(type(pathOrRaw) == "string") then
			error(dgsGenAsrt(pathOrRaw, "dgsCreateSVG", 3, "string"))
		end
...
function dgsSVGGetDocument(svg)
	if not(dgsIsType(svg, "svg")) then
		error(dgsGenAsrt(svg, "dgsSVGGetDocument", 1, "svg"))
	end
...
function dgsSVGMarkUpdate(ele)
	if not dgsGetPluginType(ele) == "dgs-dxsvg" then
		return false
	end
...
function dgsSVGGetSize(svg)
	if not(dgsIsType(dgsEle, "svg")) then
		error(dgsGenAsrt(dgsEle, "dgsSVGGetSize", 1, "svg"))
	end
...
function dgsSVGSetSize(svg, w, h)
	if not(dgsIsType(dgsEle, "svg")) then
		error(dgsGenAsrt(dgsEle, "dgsSVGSetSize", 1, "svg"))
	end

	if not type(w) == "number" then
		error(dgsGenAsrt(w, "dgsSVGSetSize", 2, "number"))
	end

	if not type(h) == "number" then
		error(dgsGenAsrt(h, "dgsSVGSetSize", 3, "number"))
	end
...
function dgsSVGNodeSetAttributes(svgEle, attributeWithData)
	if type(svgEle) == "table" then
		for i = 1, #svgEle do
			dgsSVGNodeSetAttributes(svgEle[i], attributeWithData)
		end
