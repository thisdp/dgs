...
function dgsTooltipApplyTo(tooltip, ele, text, font, maxWidth)
	if not(dgsGetType(tooltip) == "dgs-dxtooltip") then
		error(dgsGenAsrt(tooltip, "dgsTooltipApplyTo", 1, "plugin dgs-dxtooltip"))
	end

	if not(dgsIsType(ele)) then
		error(dgsGenAsrt(ele, "dgsTooltipApplyTo", 2, "dgs-dxelement"))
	end
...
function dgsTooltipRemoveFrom(ele)
	if not(dgsIsType(ele)) then
		error(dgsGenAsrt(ele, "dgsTooltipRemoveFrom", 1, "dgs-dxelement"))
	end
...
function DGSTooltipRender()
	if not isElement(showingToolTip) then
		return dgsRemoveFastEventHandler("onDgsRender", "DGSTooltipRender")
	end
