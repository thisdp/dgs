...
local getElementID = function(ele)
	return isElement(ele) and _getElementID(ele) or tostring(ele)
end
...
function dgsAddToBackEndRenderList(dgsEle)
	if not(dgsIsType(dgsEle)) then
		error(dgsGenAsrt(dgsEle, "dgsAddToBackEndRenderList", 1, "dgs-dxelement"))
	end

	local pluginType = dgsGetPluginType(dgsEle)
	local dgsType = dgsGetType(dgsEle)

	if not(dgsBackEndRenderer[pluginType] or dgsBackEndRenderer[dgsType]) then
		error(dgsGenAsrt(dgsEle, "dgsAddToBackEndRenderList", 1, _, _, _, "Type "..pluginType.." ("..dgsType..") doesn't have back-end renderer"))
	end
...
function dgsRemoveFromBackEndRenderList(dgsEle)
	if not(dgsIsType(dgsEle)) then
		error(dgsGenAsrt(dgsEle, "dgsRemoveFromBackEndRenderList", 1, "dgs-dxelement"))
	end
...
function renderGUI(source, mx, my, enabledInherited, enabledSelf, rndtgt, xRT, yRT, xNRT, yNRT, OffsetX, OffsetY, parentAlpha)
	...
	if visible and visibleInherited and isElement(source) then
		...
		if dgsPreRendererFunction then
			dgsPreRendererFunction(source)
		end	--Process something before render
		...
		if eleType == "dgs-dxscrollbar" then							--For scrollpane	(should be optimised soon)
			...
			if isElement(attachedToParent) and dgsElementData[attachedToParent] then
				if not dgsElementData[attachedToParent].visible then
					return
				end
		...
		if eleDataP and eleDataP.mainRT == rndtgt and rndtgt then
			xRT, yRT = x, y
		end
		...
		if eleData.PixelInt then
			xRT, yRT, w, h = xRT-xRT%1, yRT-yRT%1, w-w%1, h-h%1
		end
		...
		if dgsRendererFunction then
			...
			if debugMode and isElement(source) then
				if not dgsElementData[source].debugData then
					dgsElementData[source].debugData = {}
				end
...
function onClientKeyCheck(button, state)
	if state and button:sub(1, 5) ~= "mouse" then
		if isTimer(KeyHolder.Timer) then
			killTimer(KeyHolder.Timer)
		end
	...
	if isElement(MouseData.topScrollable) then
		if MouseData.MoveScale[0] then
			...
			if dgsGetType(scalepane) == "dgs-dxscalepane" then
				...
				if resolution[1]-relSizX ~= 0 then
					xScr = -posX/(resolution[1]-relSizX)
				end

				if resolution[2]-relSizY ~= 0 then
					yScr = -posY/(resolution[2]-relSizY)
				end
	...
	if state == "down" then
		if (button == "left" and canLeftClick) or (button == "right" and canRightClick) or (button == "middle" and canMiddleClick) then
			if not MouseHolder.lastKey then
				if isTimer(MouseHolder.Timer) then
					killTimer(MouseHolder.Timer)
				end
	...
	if dgsType then
		...
		if isAlive then
			...
			if MouseData.click.left == source then
				MouseData.click.left = nil
			end

			if MouseData.click.middle == source then
				MouseData.click.middle = nil
			end

			if MouseData.click.right == source then
				MouseData.click.right = nil
			end

			dgsTriggerEvent("onDgsDestroy", source)
		end

		if eleData.attachedToGridList and isAlive then
			dgsDetachFromGridList(source)
		end
...
addEventHandler("onClientClick", root, function(button, state, x, y)
	...
	if isElement(dgsEle) then	--If clicked on a dgs element
		...
		if isCoolingDown or wasEventCancelled() then
			return
		end	--event was cancelled or was cooling down
		...
		if mouseClicked then
			...
			if dgsOnMouseClickAction[dgsEleType] then
				dgsOnMouseClickAction[dgsEleType](dgsEle, button, state)
			end
		end
		...
		if isTimer(multiClick[button][state][3]) then
			killTimer(multiClick[button][state][3])
		end

		if multiClick[button][state][1] == 0 then
			multiClick[button][state][2] = dgsEle
		end
...
function dgsApplyGeneralProperties(source, theResource)
	...
	if not dgsElementData[source] then
		dgsElementData[source] = {}
	end
